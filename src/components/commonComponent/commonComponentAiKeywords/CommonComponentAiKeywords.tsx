...
const CommonComponentAiKeyword = ({
    ...
    return (
        <div className="rounded-none border border-zinc-700 bg-zinc-900 p-2">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                    <button
                        ...
                        className="inline-flex h-6 items-center gap-0.5 rounded-none border border-indigo-600 bg-indigo-600 px-1.5 text-[10px] font-semibold uppercase tracking-wide text-white hover:bg-indigo-9500"
                        title="Add keyword"
                    >
                    ...
                    <button
                        ...
                        className="inline-flex h-6 w-6 items-center justify-center rounded-none border border-zinc-700 bg-zinc-950 text-zinc-400 hover:bg-zinc-800"
                        title="Refresh"
                    >
            ...
            {adding && (
                <div className="mb-2 flex gap-1">
                    <input
                        ...
                        className="min-w-0 flex-1 rounded-none border border-zinc-700 bg-zinc-950/80 px-2 py-1 text-xs text-zinc-100 focus:border-indigo-600 focus:bg-zinc-900 focus:outline-none"
                        maxLength={256}
                    />
                    <button
                        type="button"
                        disabled={saving}
                        onClick={submitAdd}
                        className="rounded-none border border-zinc-700 bg-zinc-900 px-2 py-1 text-[11px] font-medium text-zinc-100 hover:bg-zinc-800 disabled:opacity-50"
                    >
                        {saving ? '…' : 'Save'}
                    </button>
                </div>
            ...
            {!loading && displayKeywords.length > 0 && (
                <ul className="flex flex-wrap gap-1" role="list">
                    {displayKeywords.map((item) => (
                        <li
                            key={item._id}
                            className="max-w-full border border-indigo-700 bg-indigo-950/50 px-1.5 py-0.5 text-[10px] font-medium leading-snug text-zinc-100"
                        >
            ...
            {!loading && metaSource && (
                (metaSource.aiCategory ||
                    ...
                    <div className="mt-2 border border-zinc-700 bg-zinc-950/80 p-1.5">
                        <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px] text-zinc-100">
                            {metaSource.aiCategory ? (
                                <div>
                            ...
                            {metaSource.aiSubCategory ? (
                                <div>
                                    <span className="font-semibold text-zinc-400">Sub</span>{' '}
                            ...
                            {metaSource.aiSubTopic ? (
                                <div>
                                    <span className="font-semibold text-zinc-400">Sub-topic</span>{' '}
