...
const CodeBlock = ({
  ...
  // Inline code
  return (
    <code className="px-1.5 py-0.5 mx-0.5 rounded bg-zinc-800 text-rose-400 font-mono text-[0.8125em] break-all">
      {children}
    </code>
  );
};
...
const TableWrapper = ({ children }: { children?: ReactNode }) => (
  <div className="my-4 w-full overflow-hidden rounded-lg border border-zinc-700 shadow-sm">
    <div className="overflow-x-auto">
...
const TableHead = ({ children }: { children?: ReactNode }) => (
  <thead className="bg-zinc-950">{children}</thead>
);

const TableBody = ({ children }: { children?: ReactNode }) => (
  <tbody className="divide-y divide-zinc-800 bg-zinc-900">{children}</tbody>
);

const TableRow = ({ children }: { children?: ReactNode }) => (
  <tr className="hover:bg-zinc-800 transition-colors">{children}</tr>
);

const TableHeader = ({ children }: { children?: ReactNode }) => (
  <th className="px-3 py-2 md:px-4 md:py-3 text-left font-semibold text-zinc-100 text-xs md:text-sm">
    {children}
  </th>
);

const TableCell = ({ children }: { children?: ReactNode }) => (
  <td className="px-3 py-2 md:px-4 md:py-3 text-zinc-400 text-xs md:text-sm">
    {children}
  </td>
);
...
const Blockquote = ({ children }: { children?: ReactNode }) => (
  <blockquote className="my-4 pl-4 border-l-4 border-emerald-500 bg-emerald-950/40 py-3 pr-4 rounded-r-lg text-zinc-300 italic">
    {children}
  </blockquote>
);
...
const H2 = ({ children }: { children?: ReactNode }) => (
  <h2 className="text-xl font-bold text-zinc-100 mt-5 mb-3 pb-1 border-b border-zinc-800">
    {children}
  </h2>
);

const H3 = ({ children }: { children?: ReactNode }) => (
  <h3 className="text-lg font-semibold text-zinc-100 mt-4 mb-2">{children}</h3>
);
...
const UnorderedList = ({ children }: { children?: ReactNode }) => (
  <ul className="my-3 ml-4 space-y-1 list-disc list-outside text-zinc-300">
    {children}
  </ul>
);

const OrderedList = ({ children }: { children?: ReactNode }) => (
  <ol className="my-3 ml-4 space-y-1 list-decimal list-outside text-zinc-300">
    {children}
  </ol>
);
...
const HorizontalRule = () => (
  <hr className="my-6 border-t-2 border-zinc-700" />
);
