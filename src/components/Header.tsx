...
const Header = () => {
    ...
    const navPill =
        'rounded-md px-2 py-1 text-xs font-medium text-zinc-300 transition hover:bg-zinc-800 hover:text-zinc-100';

    return (
        <header
            className={`fixed w-full border-b transition-colors duration-200 ${
                isScrolled
                    ? 'border-zinc-700/90 bg-zinc-900/95 shadow-sm backdrop-blur-sm'
                    : 'border-zinc-700/70 bg-zinc-900/90 backdrop-blur-sm'
            }`}
        ...
        >
            <div className="container mx-auto flex h-full items-center justify-between px-2 sm:px-3">
                <Link
                    to="/"
                    className="text-sm font-bold tracking-tight text-zinc-100 transition hover:text-teal-400 sm:text-base"
                >
                    AI Notes <span className="font-semibold text-teal-400">XYZ</span>
                </Link>
                <nav className="hidden items-center gap-0.5 lg:flex">
                    ...
                    {authState.isLoggedIn === 'true' ? (
                        <Link
                            to="/logout"
                            className="rounded-md px-2 py-1 text-xs font-semibold text-rose-400 transition hover:bg-rose-950"
                        >
                            Logout
                        </Link>
                    ) : (
                ...
                <div className="flex items-center gap-0.5 lg:hidden">
                    <Link
                        to="/user/search"
                        className="rounded-md p-1.5 text-zinc-400 transition hover:bg-zinc-800"
                    >
                    ...
                    <button
                        type="button"
                        className="rounded-md p-1.5 text-zinc-200 transition hover:bg-zinc-800"
