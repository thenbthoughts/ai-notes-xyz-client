    ...
    return (
        <div>
            ...
            <div className="mt-2 flex min-w-0 flex-wrap items-center justify-end gap-2">
                ...
                {/* Copy Button */}
                <button
                    className="text-sm bg-zinc-800 text-zinc-100 text-sm font-semibold hover:bg-zinc-700 p-2 mt-1 rounded-sm mr-2"
                    onClick={async () => {
                ...
                <button
                    className="text-sm bg-zinc-800 text-zinc-100 text-sm font-semibold hover:bg-zinc-700 p-2 mt-1 rounded-sm"
                    onClick={async () => {
                        const quill = quillRef.current?.getEditor?.();
                        if (quill) {
                            const markdownContent = await htmlToMarkdown({
