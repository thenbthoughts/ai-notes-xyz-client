...
import ComponentNotesCommentListAudioInput from './ComponentNotesCommentListAudioInput';
...
const ComponentNotesCommentListFileUpload = ({
    notesId,
    ...
    const uploadFiles = async (fileList: FileList | File[]) => {
        ...
        setUploading(false);
        setNotesCommentsReloadRandomNumCurrent(prev => prev + 1);
...
const ComponentNotesCommentItem = ({
    comment,
    setNotesCommentsReloadRandomNumCurrent,
}: {
    ...
    const deleteComment = async (id: string) => {
        try {
            await axiosCustom.post(
                '/api/notes-comments/crud/notesCommentDelete',
                {
                    id
                }
            );
...
const ComponentNotesCommentAdd = ({
    notesId,
    setNotesCommentsReloadRandomNumCurrent,
}: {
    notesId: string;
...
const ComponentNotesCommentList: React.FC<{
    parentNotesId: string;
}> = ({ parentNotesId }) => {
    const [comments, setCommands] = useState<NotesComment[]>([]);
    const [loading, setLoading] = useState(true);

    const [notesCommentsReloadRandomNumCurrent, setNotesCommentsReloadRandomNumCurrent] = useState(0);

    useEffect(() => {
        fetchCommands();
    }, [parentNotesId, notesCommentsReloadRandomNumCurrent]);

    const fetchCommands = async () => {
        setLoading(true);
        try {
            const response = await axiosCustom.post('/api/notes-comments/crud/notesCommentGet', { notesId: parentNotesId });
    ...
    return (
        <div>
            ...
            {/* upload file */}
            <div>
                <ComponentNotesCommentListFileUpload
                    notesId={parentNotesId}
                    setNotesCommentsReloadRandomNumCurrent={setNotesCommentsReloadRandomNumCurrent}
                />
            </div>

            {/* audio input */}
            <div>
                <ComponentNotesCommentListAudioInput
                    notesId={parentNotesId}
                    setNotesCommentsReloadRandomNumCurrent={setNotesCommentsReloadRandomNumCurrent}
                />
            </div>
...
export default ComponentNotesCommentList;
