...
interface IInfoVaultFileUpload {
    _id: string;
    fileType: string;
    fileUrl: string;
    fileTitle: string;
...
const ComponentInfoVaultEditFileUpload = ({ infoVaultId }: { infoVaultId: string }) => {
    const [files, setFiles] = useState<IInfoVaultFileUpload[]>([]);
    ...
    useEffect(() => { fetchFiles(); }, [infoVaultId]);

    // Upload files
    const uploadFiles = async (fileList: FileList | File[]) => {
        if (!fileList || !infoVaultId) return;
        setUploading(true);
...
export default ComponentInfoVaultEditFileUpload;