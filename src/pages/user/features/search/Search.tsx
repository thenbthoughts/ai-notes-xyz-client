...
const ComponentTaskWorkspaceSelect = ({
    ...
    return (
        <Fragment>
            ...
            {workspaces.length === 0 && (
                <div className="p-2 border border-zinc-700 rounded-sm hover:bg-zinc-800 block w-full focus:outline-none focus:ring-2 focus:ring-indigo-500 text-zinc-300">
                    No workspaces found
                </div>
            )}
        </Fragment>
...
const Search = () => {
    ...
    const renderFilters = () => {
        return (
            <div className="bg-zinc-900 border border-zinc-700 rounded-sm p-4 space-y-4">
                ...
                {showFilters && (
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-zinc-300">Content Types:</label>
                            <div className="flex flex-wrap gap-3">
                                ...
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input
                                        ...
                                        className="rounded"
                                    />
                                    <span className="text-sm text-zinc-300">Life Events</span>
                                </label>
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        ...
                                        className="rounded"
                                    />
                                    <span className="text-sm text-zinc-300">Info Vault</span>
                                </label>
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        ...
                                        className="rounded"
                                    />
                                    <span className="text-sm text-zinc-300">Chat LLM</span>
                                </label>
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        ...
                                        className="rounded"
                                    />
                                    <span className="text-sm text-zinc-300">Memo</span>
                                </label>
                            </div>
                        </div>
                        ...
                        {filters.filterEventTypeTasks && (
                            <div className="space-y-3 border border-zinc-700 rounded-sm p-3">
                                <h4 className="text-sm font-semibold text-zinc-300">Task Filters</h4>

                                <div>
                                    <label className="text-sm font-medium text-zinc-300">Task Status:</label>
                                ...
                                <div>
                                    <label className="text-sm font-medium text-zinc-300">Archive Status:</label>
                                    <div className="flex gap-2 mt-1">
                                        <button
                                ...
                                <div>
                                    <label className="text-sm font-medium text-zinc-300">Task Workspace:</label>
                        ...
                        {filters.filterEventTypeNotes && (
                            <div className="space-y-3 border border-zinc-700 rounded-sm p-3">
                                <h4 className="text-sm font-semibold text-zinc-300">Notes Filters</h4>

                                <div>
                                    <label className="text-sm font-medium text-zinc-300">Notes Workspace:</label>
                        ...
                        {filters.filterEventTypeLifeEvents && (
                            <div className="space-y-3 border border-zinc-700 rounded-sm p-3">
                                <h4 className="text-sm font-semibold text-zinc-300">Life Event Filters</h4>
                                <div>
                                    <label className="text-sm font-medium text-zinc-300">Life Event Search Diary:</label>
                                    <div className="flex gap-2 mt-1">
                                        <label className="flex items-center space-x-2 cursor-pointer">
                                            <input
                                                type="checkbox"
                                                ...
                                                className="rounded"
                                            />
                                            <span className="text-sm text-zinc-300">Show Diary</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
    ...
    const renderResultItem = (item: SearchResultItem) => {
        ...
        let eventTypeColor = 'bg-zinc-800 text-zinc-200';
        let isStar = false;
        let status = '';
        let priority = '';
        let eventImpact = '';
        ...
        if (item.collectionName === 'memoNotes' && item.memoInfo) {
            ...
            itemLink = '/user/memo';
            eventTypeLabel = 'Memo';
            eventTypeColor = 'bg-amber-950 text-amber-200';
        }

        return (
            <div
                key={item._id}
                className="bg-zinc-900 border border-zinc-700 rounded-sm p-4 hover:shadow-md transition-shadow"
            >
                <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                        ...
                        {/* Title */}
                        <h4 className="text-base font-semibold text-zinc-100 mb-2 line-clamp-2">
                            {title}
                        </h4>
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-400">
                            ...
                            {item.collectionName === 'tasks' && item.taskInfo && (
                                <Fragment>
                                    ...
                                    {item.taskInfo.isArchived && (
                                        <span className="px-2 py-0.5 rounded-sm bg-zinc-800 text-zinc-400">
                                            Archived
                                        </span>
                                    )}
                                </Fragment>
                            ...
                            {item.collectionName === 'memoNotes' && item.memoInfo?.pinned && (
                                <span className="px-2 py-0.5 rounded-sm bg-amber-950 text-amber-300">
                                    Pinned
                                </span>
                            )}
                            ...
                            {(labels.length > 0 || tags.length > 0) && (
                                <Fragment>
                                    ...
                                    {tags.slice(0, 5).map((tag: string, idx: number) => (
                                        <span
                                            key={`tag-${idx}`}
                                            className="px-2 py-0.5 bg-zinc-800 text-zinc-200 rounded"
                                        >
                                            {tag}
                                        </span>
                                    ))}
            ...
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="mb-6 flex items-start justify-between">
                    <div>
                        ...
                        <p className="text-zinc-400">Search across tasks, notes, memos, life events, chats, and more</p>
                    </div>
                    <button
                ...
                <div className="mb-6">
                    <div className="relative">
                        <LucideSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 text-zinc-500 w-5 h-5" />
                        <input
                            type="text"
                            placeholder="Search for anything..."
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            className="w-full pl-12 pr-4 py-3 border border-zinc-700 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-lg bg-zinc-900 text-zinc-100 placeholder:text-zinc-500"
                            autoFocus
                        />
                    </div>
                </div>
                ...
                {/* Results Count */}
                <div className="mb-4">
                    <div className="text-sm text-zinc-400 bg-zinc-900 border border-zinc-700 rounded-sm px-4 py-2">
                        {isLoading ? (
                            <span>Loading...</span>
                        ) : (
                            <span>
                ...
                {list.length === 0 && !isLoading && (
                    <div className="text-center py-12 bg-zinc-900 border border-zinc-700 rounded-sm">
                        <LucideSearch className="w-16 h-16 text-zinc-600 mx-auto mb-4" />
                ...
                {totalCount > perPage && (
                    <div className="flex justify-center items-center gap-3 mt-6">
                        <button
                            ...
                            disabled={page === 1}
                            className="px-4 py-2 border border-zinc-700 rounded-sm hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-zinc-900 transition-colors bg-zinc-900 text-zinc-300"
                        >
                            ← Previous
                        </button>
