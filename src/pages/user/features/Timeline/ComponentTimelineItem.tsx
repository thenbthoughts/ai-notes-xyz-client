...
const ComponentTimelineItem = ({ item }: { item: TimelineItem }) => {
    ...
    return (
        <div
            onClick={handleClick}
            className={`cursor-pointer border-l-2 bg-zinc-900 px-3 py-2 transition-colors hover:bg-zinc-800 ${getEntityColor(
                item.entityType
            )}`}
        >
                ...
                <div className="min-w-0 flex-1">
                    <div className="mb-0.5 flex flex-wrap items-center gap-2">
                        ...
                        {(item.fileType || (item.mediaAttachments && item.mediaAttachments.length > 0)) && (
                            <span className="rounded bg-orange-900/50 px-1.5 py-0.5 text-[10px] font-medium capitalize text-orange-300">
                                {item.fileType
                    ...
                    {item.title && (
                        <h3 className="mb-0.5 truncate text-xs font-medium text-zinc-100">
                            {item.title}
                        </h3>
                    )}

                    {item.content && (
                        <p className="line-clamp-2 text-[10px] text-zinc-400">{item.content}</p>
                    )}

                    {!item.title && !item.content && !item.fileType && (
                        <p className="truncate text-[10px] text-zinc-400">{label}</p>
                    )}
