...
const PhoneItem = ({
    ...
    return (
        <div className="border border-zinc-700 bg-zinc-900 p-3 rounded-sm hover:shadow-sm transition-shadow">
            <div className="flex flex-col gap-2">
                <div className="flex flex-col sm:flex-row gap-2">
                    <DebounceInput
                        ...
                        className="sm:w-32 px-2 sm:px-3 py-2 border border-zinc-700 bg-zinc-900 text-zinc-100 rounded-sm text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-colors"
                    />
                    <DebounceInput
                        debounceTimeout={1000}
                        type="tel"
                        ...
                        className="flex-1 px-2 sm:px-3 py-2 border border-zinc-700 bg-zinc-900 text-zinc-100 rounded-sm text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-colors"
                    />
                    <DebounceInput
                        debounceTimeout={1000}
                        type="text"
                        ...
                        className="sm:w-40 px-2 sm:px-3 py-2 border border-zinc-700 bg-zinc-900 text-zinc-100 rounded-sm text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-400 transition-colors"
                    />
                </div>
                ...
                <div className="flex items-center gap-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            ...
                            className="w-4 h-4 text-blue-600 bg-zinc-800 border-zinc-700 rounded-sm focus:ring-blue-500 focus:ring-2"
                        />
            ...
            <div className="flex gap-1 mt-2 flex-wrap">
                <a href={`tel:${formData.countryCode}${formData.phoneNumber}`} aria-label="Call phone" className="text-zinc-400 hover:text-zinc-100 p-1 text-xs border border-zinc-700 rounded-sm">tel:</a>
                <button type="button" aria-label="Copy phone" className="text-zinc-400 hover:text-zinc-100 p-1 text-xs border border-zinc-700 rounded-sm" onClick={async () => { try { await navigator.clipboard.writeText(`${formData.countryCode}${formData.phoneNumber}`); toast.success('Copied'); } catch { toast.error('Copy failed'); } }}>Copy</button>
                <button
...
const ComponentInfoVaultPhone = ({ infoVaultId }: { infoVaultId: string }) => {
    ...
    return (
        <div className="space-y-3">
            ...
            {/* Phones List */}
            {loading ? (
                <div className="text-zinc-400 text-sm py-4 text-center">Loading phones...</div>
            ) : (
                <div className="space-y-2">
                    {phones.length === 0 && (
                        <div className="text-zinc-400 text-sm py-4 text-center bg-zinc-950 rounded-sm border border-zinc-700">
                            No phones added yet.
                        </div>
                    )}
                    {phones.map((phone) => {
