    ...
    return (
        <div className="border border-zinc-700 bg-zinc-900 p-3 rounded-sm hover:shadow-sm transition-shadow">
            ...
            <div className="flex gap-1 mt-2 flex-wrap">
                <a href={`mailto:${formData.email}`} aria-label="Send email" className="text-zinc-400 hover:text-zinc-100 p-1 text-xs border border-zinc-700 rounded-sm">mailto:</a>
                <button type="button" aria-label="Copy email" className="text-zinc-400 hover:text-zinc-100 p-1 text-xs border border-zinc-700 rounded-sm" onClick={async () => { try { await navigator.clipboard.writeText(formData.email); toast.success('Copied'); } catch { toast.error('Copy failed'); } }}>Copy</button>
                <button
...
const ComponentInfoVaultEmail = ({ infoVaultId }: { infoVaultId: string }) => {
    ...
    return (
        <div className="space-y-3">
            ...
            {/* Emails List */}
            {loading ? (
                <div className="text-zinc-400 text-sm py-4 text-center">Loading emails...</div>
            ) : (
