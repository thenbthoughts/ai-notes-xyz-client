...
import { jotaiChatHistoryModalOpen, jotaiChatLlmThreadSetting, jotaiChatSearchVisible, jotaiHideSidebar } from '../jotai/jotaiChatLlmThreadSetting.ts';
...
const railNeutral =
    'border border-zinc-700/80 bg-zinc-900/50 text-zinc-400 shadow-[0_2px_8px_-2px_rgba(15,23,42,0.07),inset_0_1px_0_0_rgba(39,39,42,0.95)] ring-1 ring-zinc-950/[0.04] backdrop-blur-md hover:-translate-y-px hover:bg-zinc-900/85 hover:text-zinc-100 hover:shadow-[0_10px_24px_-8px_rgba(15,23,42,0.14)]';

/** Teal → cyan gradient + glow (distinct from flat fills) */
const railActive =
    'border border-teal-400/25 bg-gradient-to-br from-teal-500 via-teal-600 to-cyan-600 text-white shadow-[0_6px_22px_-5px_rgba(13,148,136,0.42),inset_0_1px_0_0_rgba(39,39,42,0.22)] ring-1 ring-zinc-700/15 hover:-translate-y-px hover:from-teal-400 hover:via-teal-500 hover:to-cyan-500 hover:shadow-[0_10px_28px_-6px_rgba(13,148,136,0.5)]';

/** Primary action — brighter specular + motion */
const railCta =
    'border border-zinc-700/25 bg-gradient-to-br from-teal-500 via-emerald-500 to-cyan-500 text-white shadow-[0_8px_26px_-6px_rgba(5,150,105,0.48),inset_0_1px_0_0_rgba(39,39,42,0.35)] ring-1 ring-zinc-700/20 hover:-translate-y-0.5 hover:from-teal-400 hover:via-emerald-400 hover:to-cyan-400 hover:shadow-[0_14px_36px_-8px_rgba(5,150,105,0.55)]';
...
const ChatRightFilterWrapper = ({
    ...
    const [searchVisible, setSearchVisible] = useAtom(jotaiChatSearchVisible);
    ...
    return (
        <div className="flex w-full flex-col gap-1.5 px-1">
            ...
            <button
                type="button"
                className={`${railLayout} ${searchVisible ? railActive : railNeutral}`}
                onClick={() => {
            ...
            >
                <LucideSearch className={iconClass} strokeWidth={1.75} />
            </button>
