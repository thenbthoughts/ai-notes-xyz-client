...
export default function AuthenticatedStoredArtifactRow({
    ...
    const caption = (
        <div className="mb-1 flex flex-wrap items-center gap-2">
            ...
            {purpose ? (
                <span className="rounded-full bg-zinc-800 px-2 py-0.5 text-[10px] text-zinc-400">{purpose}</span>
            ) : null}
        </div>
    );
    ...
    const actionButtons = (
        <div className="flex shrink-0 flex-wrap gap-2">
            <button
                ...
                className="text-xs font-medium text-teal-400 hover:text-teal-300 hover:underline disabled:cursor-not-allowed disabled:opacity-50"
            >
                Open
            </button>
            <button
                ...
                className="text-xs font-medium text-zinc-400 hover:text-zinc-200 hover:underline disabled:opacity-50"
            >
    ...
    if (isPdf || isHtml) {
        return (
            <div className="overflow-hidden rounded-xl border border-zinc-700/80 bg-zinc-950/60">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-700/60 bg-zinc-900/80 px-2 py-1.5">
                    {caption}
                    {actionButtons}
                </div>
                {previewLoading ? (
                    ...
                    <iframe
                        title={name}
                        src={blobUrl}
                        className="block h-[min(50vh,420px)] w-full bg-zinc-900"
                ...
                {description ? (
                    <p className="border-t border-zinc-700/50 px-2 py-1 text-[11px] text-zinc-500">{description}</p>
                ) : null}
            </div>
        );
    }
            ...
            <div className="rounded-xl border border-zinc-700/80 bg-zinc-900/70 p-2">
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <button
                        ...
                        disabled={downloadBusy}
                        onClick={() => void onDownload()}
                        className="shrink-0 text-xs font-medium text-teal-400 hover:underline disabled:opacity-50"
                    >
    ...
    return (
        <div className="rounded-lg border border-zinc-700/70 bg-zinc-950/80 px-3 py-2">
            {caption}
            <button
                type="button"
                disabled={downloadBusy}
