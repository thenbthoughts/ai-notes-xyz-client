/**
 * AM3 pipeline rows use a synthetic iteration key `synth-{requestId}-{iterationNumber}` from notesGet;
 * legacy payloads may still use a Mongo iteration id string on both iteration and sub_question rows.
 */
import type { AnswerMachineV3StreamPayload, tsMessageItem } from '../../../../../../types/pages/tsNotesAdvanceList';

export type Am3PipelineRenderBlock =
    | {
...
function subRowIterationKey(sub: tsMessageItem, iterations: tsMessageItem[]): string | undefined {
    ...
    const syntheticFallback =
        sp.requestId != null && sp.iterationNumber != null
            ? `synth-${sp.requestId}-${sp.iterationNumber}`
            : undefined;
    ...
    if (docId) {
        ...
        if (iterKeyMatches || docId.startsWith('synth-')) {
            return docId;
        }
...
function sortSubs(a: tsMessageItem, b: tsMessageItem): number {
    const sa = a.streamPayload;
    const sb = b.streamPayload;
...
function stableSubQuestionDocId(sq: tsMessageItem): string {
    ...
    return id.startsWith('am3-sq-') ? id.slice('am3-sq-'.length) : id;
}

/**
 * Builds iteration-centric blocks (iteration + nested sub-questions) and final-answer rows
 * from a chronologically ordered chunk of `answer_machine_v3_stream` messages. Sub-questions and
 * files group by the same iteration key (`synth-…` or legacy id). File artifact rows are wired to
 * iterations/sub-questions when IDs are present; otherwise they ride as loose attachments on the
 * matching request's final answer card or fall back to an orphan bucket.
 */
export function groupAnswerMachineV3PipelineItems(items: tsMessageItem[]): Am3PipelineRenderBlock[] {
    ...
    orderedIterations.forEach((iter, idx) => {
        ...
        if (sp.requestId != null && lastIterIndexByRequest.get(sp.requestId) === idx && finalsByRequest.has(sp.requestId)) {
