                ...
                {loading ? (
                    ...
                    <ul className="max-h-[30vh] space-y-2 overflow-auto pr-0.5">
                        {questions.map((question, index) => (
                            <li
                                key={index}
                                className="flex items-start justify-between gap-2 rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-3 text-sm text-zinc-200"
                            >
                                <span className="min-w-0 flex-1 leading-snug">{question}</span>
                                <button
                                    type="button"
                                    onClick={() => void navigator.clipboard.writeText(question)}
                                    className="shrink-0 rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1 text-[11px] font-medium text-teal-400 shadow-sm transition hover:border-teal-200 hover:bg-teal-950/40"
                                >
                                    Copy
                                </button>
                            </li>
        ...
        return (
            <div>
                <div className="space-y-3">
                    <div className="rounded-xl border border-violet-700/70 bg-gradient-to-br from-violet-950/40 to-teal-950/40 p-3">
                        <p className="text-sm font-semibold text-zinc-100">{threadInfo.threadTitle}</p>
                    </div>

                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        <div className="rounded-xl border border-emerald-700/70 bg-emerald-950/40 p-2.5">
                            <div className="flex items-center gap-2">
                                <div
                                    className={`h-2 w-2 rounded-full ${threadInfo.isPersonalContextEnabled ? 'bg-emerald-9500' : 'bg-zinc-300'}`}
                                />
                                <span className="text-xs font-medium text-zinc-300">Personal context</span>
                            </div>
                        </div>

                        <div className="rounded-xl border border-amber-700/70 bg-amber-950/40 p-2.5">
                            <div className="flex items-center gap-2">
                                <div
                                    className={`h-2 w-2 rounded-full ${threadInfo.isAutoAiContextSelectEnabled ? 'bg-amber-950/400' : 'bg-zinc-300'}`}
                                />
                                <span className="text-xs font-medium text-zinc-300">Auto AI context</span>
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center justify-between rounded-xl border border-indigo-700/70 bg-indigo-950/40 p-3">
                        ...
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-indigo-900/40">
                            <div className="h-2 w-2 rounded-full bg-indigo-9500" />
                        </div>
                    </div>

                    {threadInfo.aiSummary && (
                        <div className="rounded-xl border border-zinc-700/80 bg-zinc-950/80 p-3">
                            <p className="text-xs leading-relaxed text-zinc-400">{threadInfo.aiSummary}</p>
                        </div>
                    )}

                    {threadInfo.tagsAi && threadInfo.tagsAi.length > 0 && (
                        <div className="rounded-xl border border-violet-700/70 bg-violet-950/40 p-3">
                            <div className="flex flex-wrap gap-1.5">
                                {threadInfo.tagsAi.map((tag, index) => (
                                    <span
                                        key={index}
                                        className="rounded-md border border-violet-700/80 bg-violet-900/40 px-2 py-0.5 text-xs font-medium text-violet-300"
                                    >
                                        {tag}
                                    </span>
                                ))}
    ...
    return (
        <div className="py-4">
            <div className="px-1 sm:px-0">
                <div className="w-full rounded-2xl border border-zinc-700/80 bg-zinc-900/90 p-4 shadow-lg shadow-zinc-900/[0.04] ring-1 ring-zinc-700/[0.02] backdrop-blur-sm">
