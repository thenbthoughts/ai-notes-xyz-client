...
function ShellRunImportedFilePreviews({
    ...
    return (
        <div className="mt-4 space-y-4 border-t border-zinc-700/70 pt-3">
...
function AgentCitationChips({
    artifact,
}: {
    artifact: NonNullable<tsMessageItem['agentFinalArtifactV1']>;
}) {
    const [openId, setOpenId] = useState<string | null>(null);
    const citations = artifact.citations || [];
    if (!citations.length && !artifact.researchBrief) {
        return null;
    }

    const sourcePath = (source: string, id: string): string | null => {
        if (source === 'notes') return `/user/notes?id=${id}`;
        if (source === 'tasks') return `/user/task?id=${id}`;
        if (source === 'memo') return `/user/memo?id=${id}`;
        if (source === 'lifeEvents') return `/user/life-events?id=${id}`;
        if (source === 'infoVault') return `/user/info-vault?id=${id}`;
        return null;
    };

    return (
        <div className="mt-3 space-y-2 border-t border-teal-800/80 pt-2">
            {artifact.confidence === 'low' ? (
                <p className="text-[10px] text-amber-700">
                    Limited personal context found — parts marked speculative.
                </p>
            ) : null}
            {citations.length > 0 ? (
                <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                        {citations.map((c) => {
                            ...
                            const href = sourcePath(c.source, c.id);
                            ...
                            return (
                                <div key={key} className="relative">
                                    <button
                                        ...
                                        className="rounded-full border border-teal-200/80 bg-teal-950/90 px-2 py-0.5 text-[10px] font-medium text-teal-300 transition hover:bg-teal-900/40"
                                        title={c.title}
                                    >
                                        [{c.source}] {c.title.slice(0, 28)}
                                        {c.title.length > 28 ? '…' : ''}
                                    </button>
                                    {isOpen ? (
                                        <div className="absolute left-0 z-20 mt-1 w-64 rounded-lg border border-zinc-700 bg-zinc-900 p-2 shadow-lg">
                                            <p className="text-[11px] font-medium text-zinc-200">{c.title}</p>
                                            <p className="mt-1 max-h-24 overflow-y-auto text-[10px] leading-snug text-zinc-400">
                                                {c.summary || 'No preview'}
                                            </p>
                                            {href ? (
                                                <Link
                                                    to={href}
                                                    className="mt-1.5 inline-block text-[10px] font-medium text-teal-400 hover:underline"
                                                >
                                                    Open {c.source}
                                                </Link>
                                            ) : null}
            ...
            {artifact.researchBrief ? (
                <details className="rounded-md bg-zinc-950/90 px-2 py-1 ring-1 ring-zinc-700/60">
                    <summary className="cursor-pointer text-[10px] font-medium text-zinc-400">
                        Reasoning trace
                    </summary>
                    <p className="mt-1 max-h-32 overflow-y-auto whitespace-pre-wrap text-[10px] leading-snug text-zinc-400">
                        {artifact.researchBrief}
                    </p>
                </details>
            ) : null}
                                ...
                                {tasks.length > 0 ? (
                                    <ul className="mt-2 space-y-2">
                                        {tasks.map((task, index) => (
                                            ...
                                            >
                                                <h4 className="font-semibold text-zinc-100">{task.taskTitle}</h4>
                                                <p className="text-sm text-zinc-400">{task.taskDescription}</p>
...
const collectMessageFileUrls = (m: tsMessageItem): string[] => {
    const out: string[] = [];
    ...
    push(m.fileUrl);
    if (Array.isArray(m.fileUrlArr)) {
        for (const item of m.fileUrlArr) {
            push(item);
        }
    }
...
const ComponentMessageItem = ({
        ...
        return (
            <div className="pb-1">
                <img
                    ...
                    alt="Attached photo"
                    className="max-w-full rounded-xl ring-1 ring-zinc-700/5"
                    loading="lazy"
                    style={{
                        maxHeight: '30vh',
                        objectFit: 'contain'
    ...
    const renderDocument = () => {
        ...
        return (
            <div className={`text-sm ${body}`}>
                ...
                {showExtractedText && (
                    <div
                        className={`mt-2 max-h-[min(300px,80vh)] overflow-y-auto whitespace-pre-wrap rounded-lg border border-zinc-700/40 p-2 text-xs ${meta}`}
                    >
    ...
    const renderText = () => {
        ...
        const attachedFiles = collectMessageFileUrls(itemMessage);
        return (
            <div
                className={`w-full min-w-0 text-sm leading-relaxed ${
                    isAssistant
                        ? 'text-zinc-200'
                        : 'text-zinc-200 [&_a]:text-teal-400 [&_code]:text-teal-300'
                }`}
            >
                {itemMessage?.reasoningContent?.length >= 1 && (
                    <div className="mb-3 rounded-xl border border-violet-700/60 bg-violet-950/40 p-3">
                        <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-violet-600">
                            Reasoning
                        </div>
                        <span className="text-xs text-violet-300/90">{itemMessage?.reasoningContent}</span>
                    </div>
                )}
                <MarkdownRenderer content={mdContent} />
                {itemMessage.agentFinalArtifactV1 ? (
                    <AgentCitationChips artifact={itemMessage.agentFinalArtifactV1} />
                ) : null}
                {attachedFiles.length > 0 ? (
                    <div className="mt-4 space-y-4 border-t border-zinc-700/70 pt-3">
                        ...
                        {attachedFiles.map((storedFileUrl) => (
                            <AuthenticatedStoredArtifactRow
                                key={storedFileUrl}
                                storedFileUrl={storedFileUrl}
                                originalName={storedFileUrl.split('/').pop() || 'file'}
    ...
    const iconBtnAssistant = `${iconBtnBase} bg-zinc-800/90 text-zinc-400 hover:bg-zinc-700/90`;
    const iconBtnUser = `${iconBtnBase} bg-zinc-900/90 text-zinc-400 shadow-sm ring-1 ring-teal-200/40 hover:bg-teal-950/40`;
            ...
            <div className="mt-3 border-t border-zinc-700/50 pt-2">
                <div className="flex flex-wrap items-center gap-1">
                    ...
                    <button
                        ...
                        className={`${iconBtnBase} bg-red-950/40 text-red-600 hover:bg-red-900/40`}
                        title="Delete"
                    >
    ...
    const renderTags = () => {
        ...
        const tagClass = isAssistant
            ? 'rounded-md border border-teal-200/70 bg-teal-950/90 px-2 py-0.5 text-[11px] font-medium text-teal-300'
            : 'rounded-md border border-emerald-700/80 bg-emerald-950/40 px-2 py-0.5 text-[11px] font-medium text-emerald-300';
        const linkClass = isAssistant
            ? 'text-xs font-medium text-teal-600 hover:text-teal-400'
            : 'text-xs font-medium text-emerald-700 hover:text-emerald-300';
        return (
            <Fragment>
    ...
    const renderAiGeneratedFileInfo = () => {
        return (
            <Fragment>
                {showAiGeneratedFileInfo && (
                    <div className="my-2 rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-3">
                        <p className="text-xs leading-relaxed text-zinc-400">{itemMessage.fileContentAi}</p>
                    </div>
                )}
            </Fragment>
        );
    ...
    const renderUsage = () => {
        return (
            <div>
                {showUsage && (
                    <div className="my-2 rounded-xl border border-violet-700/70 bg-violet-950/40 p-3">
                        <p className="mb-1 text-xs font-semibold text-violet-300">Usage</p>
    ...
    const bubbleClass = isAssistant
        ? 'border border-zinc-700/90 bg-zinc-900/95 shadow-lg shadow-zinc-900/[0.04] ring-1 ring-zinc-700/[0.03] backdrop-blur-sm'
        : 'border border-teal-200/70 bg-gradient-to-br from-teal-950/40 to-emerald-950/30 shadow-md shadow-teal-900/[0.06] ring-1 ring-teal-800/80 backdrop-blur-sm';
    ...
    return (
        ...
        >
            {isDeleted && (
                <div className="max-w-full rounded-2xl border border-zinc-700/80 bg-zinc-800/90 px-4 py-3 text-sm text-zinc-500 shadow-inner md:max-w-[650px]">
                    Message removed
                </div>
            )}
            {!isDeleted && (
