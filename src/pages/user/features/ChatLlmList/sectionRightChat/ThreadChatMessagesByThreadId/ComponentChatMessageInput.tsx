...
const TextAndFileInput = ({
    ...
    return (
        <div className="w-full">
            <textarea
                className="w-full resize-none rounded-xl border border-zinc-700/80 bg-zinc-950/80 px-3 py-2.5 text-sm text-zinc-100 shadow-inner placeholder:text-zinc-400 transition-shadow focus:border-teal-500/50 focus:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-teal-500/20"
        ...
        <div className="w-full mb-2 overflow-x-auto pt-1">
            <div className="flex gap-2 min-w-max">
                {files.map((file, index) => {
                    ...
                    return (
                        <div key={index} className="flex-shrink-0 bg-zinc-800 p-2 rounded-sm min-w-[200opx] max-w-[200px]">
    ...
    const handleAddNote = async () => {
        ...
        try {
            ...
            if (newNote.trim().length > 1) {
                ...
                if (threadInfo.answerEngine === 'agent') {
                    await axiosCustom.post(
                        '/api/chat-llm/add-auto-next-message/agent',
                        {
                            threadId: threadId,
                        },
                        { signal },
                    ...
                    toast.success('Agent started. Polling for updates...');
                    setRefreshParentRandomNum(Math.floor(Math.random() * 1_000_000));
                } else if (threadInfo.answerEngine === 'agentOpencode') {
                    const initRes = await axiosCustom.post(
                        '/api/chat-llm/agent-opencode/init',
                        {
                            threadId: threadId,
                        },
                        { signal },
                    );
                    const sid = typeof initRes.data?.opencodeSessionId === 'string' ? initRes.data.opencodeSessionId : '';
                    toast.dismiss(noteLoadingToastId);
                    if (sid) {
                        toast.success(`Agent (Opencode) started. Session ${sid} — workspace will be initialized shortly...`);
                    } else {
                        toast.success('Agent (Opencode) started. Workspace will be initialized shortly...');
                    }
                    setRefreshParentRandomNum(Math.floor(Math.random() * 1_000_000));
                } else {
    ...
    const regenerateResponse = async () => {
        ...
        try {
            ...
            if (threadInfo.answerEngine === 'agent') {
                ...
                const initRes2 = await axiosCustom.post(
                    '/api/chat-llm/agent-opencode/init',
                    {
                        threadId: threadId,
                    },
                ...
                const sid2 = typeof initRes2.data?.opencodeSessionId === 'string' ? initRes2.data.opencodeSessionId : '';
                toast.dismiss(toastLoadingId);
                if (sid2) {
                    toast.success(`Agent (Opencode) started. Session ${sid2}`);
                } else {
                    toast.success('Agent (Opencode) started. Workspace will be initialized shortly...');
                }
            } else {
    ...
    return (
        <>
            <div
                ...
                className="border-t border-zinc-700/80 bg-gradient-to-b from-zinc-900 to-zinc-950/90 px-3 pb-2 pt-2"
            >
                ...
                <div className="flex items-stretch gap-0">
                    ...
                    >
                        ...
                        {isSubmitting && (
                            <button
                                type="button"
                                className="inline-flex h-9 shrink-0 items-center justify-center rounded-xl border border-red-200/90 bg-red-950/40 px-3 text-xs font-medium text-red-300 shadow-sm transition-colors hover:bg-red-900/40"
                        ...
                        <button
                            ...
                            className="inline-flex h-9 shrink-0 items-center gap-1 rounded-xl border border-zinc-700/90 bg-zinc-900 px-2.5 text-xs font-medium text-zinc-200 shadow-sm transition-colors hover:border-zinc-600 hover:bg-zinc-800"
                        ...
                        {isSubmitting === false && (
                            <button
                                type="button"
                                className="inline-flex h-9 shrink-0 items-center justify-center rounded-xl border border-zinc-700/90 bg-zinc-900 px-2.5 text-zinc-300 shadow-sm transition-colors hover:bg-zinc-800"
                        ...
                        <button
                            type="button"
                            className="inline-flex h-9 shrink-0 items-center rounded-xl border border-violet-700/80 bg-violet-950/40 px-2.5 text-xs font-medium text-violet-300 shadow-sm transition-colors hover:bg-violet-900/40"
                            onClick={() => {
