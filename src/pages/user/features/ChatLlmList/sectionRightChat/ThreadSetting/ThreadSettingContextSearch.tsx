...
import { reactSelectDarkStyles } from "../../component/selectModel/reactSelectPortalProps";
...
const ComponentTaskWorkspaceSelect = ({
    ...
    return (
        <Fragment>
            ...
            {workspaces.length === 0 && (
                <div className="p-2 border border-zinc-700 rounded-sm hover:bg-zinc-700 block w-full focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    No workspaces found
                </div>
            )}
        </Fragment>
        ...
        return (
            <div className="flex flex-col gap-2">
                ...
                <div className="relative">
                    <LucideSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-zinc-500 w-5 h-5" />
                    <input
                        type="text"
                        placeholder="Search contexts..."
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        className="w-full pl-10 pr-4 py-2 border border-zinc-700 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                </div>

                {/* Filter by added or not added */}
                <div className="space-y-2">
                    <label className="text-sm font-medium text-zinc-300">Context Selected:</label>
                ...
                {filters.filterEventTypeTasks && (
                    <div className="space-y-2 border border-zinc-600 rounded-sm p-1">
                        <div>
            ...
            <div className="space-y-3">
                {list.map((item) => {
                    ...
                    if (item.fromCollection === 'memo' && item.memoInfo) {
                        ...
                        itemLink = '/user/memo';
                        eventTypeLabel = 'Memo';
                        eventTypeColor = 'bg-amber-900/40 text-amber-300';
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-400">
                                        ...
                                        {item.fromCollection === 'memo' && item.memoInfo?.pinned && (
                                            <span className="px-2 py-0.5 rounded-sm bg-amber-950/40 text-amber-300">
                                                Pinned
                                            </span>
                                        )}
                                        {/* Event Impact */}
                                    ...
                                    {(labels.length > 0 || tags.length > 0) && (
                                        <div className="flex flex-wrap gap-1 mt-2">
                                            {labels.slice(0, 3).map((label: string, idx: number) => (
                                                <span
                                                    key={`label-${idx}`}
                                                    className="text-xs px-2 py-0.5 bg-zinc-800 text-blue-700 rounded"
                                                >
                                                    {label}
                                                </span>
                                            ))}
                                            {tags.slice(0, 3).map((tag: string, idx: number) => (
                                                <span
                                                    key={`tag-${idx}`}
                                                    className="text-xs px-2 py-0.5 bg-purple-950/40 text-purple-300 rounded"
                                                >
                                                    {tag}
                                                </span>
                                            ))}
            ...
            {totalCount >= perPage && (
                <div className="flex justify-center items-center gap-3 mt-6">
                    <button
                        ...
                        disabled={page === 1}
                        className="px-4 py-2 border border-zinc-700 rounded-sm hover:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-zinc-900 transition-colors"
                    >
                        ← Previous
                    </button>
