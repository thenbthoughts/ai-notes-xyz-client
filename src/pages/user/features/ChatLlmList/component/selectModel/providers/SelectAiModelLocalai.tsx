...
const SelectAiModelLocalai = ({
    ...
    const [localaiModels, setLocalaiModels] = useState<LocalaiModelDoc[]>([]);
    ...
    const filteredModels = useMemo(() => {
        ...
        return localaiModels.filter((model) => matchModalityFilters(model, modalityFilters));
    }, [localaiModels, modalityFilters]);
    ...
    const options: OptionType[] = useMemo(() => {
        return filteredModels.map((model) => {
            ...
            return {
                ...
                label: `${displayName}${typeBadge}${tokensBadge}${visionBadge}${audioBadge}${videoBadge}${embedBadge}`,
        ...
        <div className="mb-2">
            <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2">
                    <h3 className="text-xs font-medium text-zinc-300">LocalAI Model</h3>
                    {localaiModels.length > 0 && (
                        <span className="text-[10px] text-zinc-500 font-mono">
                            ({filteredModels.length}/{localaiModels.length})
                        </span>
                    )}
                </div>
                {localaiModels.length > 0 && (
            ...
            {loading ? (
                ...
                <Select<OptionType>
                    ...
                    options={options}
                    placeholder="Search LocalAI models..."
                    isLoading={loading}
                    isSearchable={true}
