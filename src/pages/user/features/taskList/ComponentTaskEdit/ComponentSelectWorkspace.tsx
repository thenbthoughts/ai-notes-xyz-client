    ...
    useEffect(() => {
        const fetchWorkspaces = async () => {
            try {
                ...
                if (modalType === 'add' && docs.length > 0) {
                    const currentInList =
                        currentWorkspaceId.length === 24 &&
                        docs.some((w) => w._id === currentWorkspaceId);
                    setWorkspaceIdFunc(currentInList ? currentWorkspaceId : docs[0]._id);
                }
            } catch (err) {
