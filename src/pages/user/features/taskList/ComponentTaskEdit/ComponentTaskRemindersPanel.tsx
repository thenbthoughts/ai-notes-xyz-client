...
const ReadOnlySendQueue = ({
    ...
    return (
        <div className="mt-2 rounded-md border border-zinc-700/70 bg-zinc-950/50 px-2 py-1.5 sm:px-2.5 sm:py-2">
...
const ComponentTaskRemindersPanel = ({
    ...
    return (
        <div className="mt-2 space-y-2">
            ...
            <section
                className="rounded-xl border border-zinc-700/60 bg-zinc-900 shadow-sm"
            ...
            >
                ...
                {expandedDue && (
                    <div className="border-t border-zinc-800 px-2 pb-2 pt-1 sm:px-2.5 sm:pb-2.5">
                        <p className="mb-1.5 text-[10px] leading-snug text-zinc-400 sm:text-[11px]">
                            Presets relative to due, plus optional exact times and cron (all require a task due date).
                        </p>
                        <div className="mb-2 flex min-w-0 flex-wrap items-center gap-1.5 rounded-lg border border-zinc-700/70 bg-zinc-950/50 px-2 py-1.5">
                            <label className="shrink-0 text-[10px] font-medium text-zinc-700 sm:text-xs">
                                Task due date
                            </label>
                            <input
                                type="datetime-local"
                                className="min-w-0 flex-1 rounded-lg border border-zinc-700/90 bg-zinc-900 py-0.5 px-1.5 text-[10px] text-zinc-100 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 sm:py-1 sm:text-xs"
                        ...
                        <div className="grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-2">
                            <div className="min-w-0 rounded-md border border-zinc-700/80 bg-zinc-900/90 p-1.5 sm:p-2">
                                ...
                                <div className="max-h-48 space-y-1 overflow-y-auto [scrollbar-width:thin]">
                                    ...
                                    {presetRows.map((row) => (
                                        <div
                                            key={`p-${row.name}`}
                                            className="flex items-start justify-between gap-1 rounded border border-zinc-700/70 bg-zinc-950/80 px-1.5 py-1 text-[10px] sm:text-[11px]"
                                        >
                                            <div className="min-w-0">
                                                <span className="font-medium text-zinc-100">Before due</span>
                                            ...
                                            <button
                                                type="button"
                                                className="shrink-0 rounded p-0.5 text-zinc-400 hover:bg-zinc-800 hover:text-red-600"
                                    ...
                                    {dueDateReminderAbsoluteTimesIso.map((iso) => (
                                        <div
                                            key={`da-${iso}`}
                                            className="flex items-start justify-between gap-1 rounded border border-indigo-700/60 bg-indigo-950/50 px-1.5 py-1 text-[10px] sm:text-[11px]"
                                        >
                                            <div className="min-w-0">
                                                <span className="font-medium text-indigo-200">Exact (due section)</span>
                                            ...
                                            <button
                                                type="button"
                                                className="shrink-0 rounded p-0.5 text-zinc-400 hover:bg-indigo-900 hover:text-red-600"
                                    ...
                                    {dueDateReminderCronExpressions.map((expr) => (
                                        <div
                                            key={`dc-${expr}`}
                                            className="flex items-start justify-between gap-1 rounded border border-zinc-700/70 bg-zinc-950/70 px-1.5 py-1 text-[10px] sm:text-[11px]"
                                        >
                                            <div className="min-w-0 font-mono text-[10px] leading-snug text-zinc-100 sm:text-[11px]">
                            ...
                            <div className="min-w-0 space-y-2 rounded-md border border-zinc-700/80 bg-zinc-900/90 p-1.5 sm:p-2">
                                ...
                                {dueValid ? (
                                    <>
                                        <div className="space-y-1 border-b border-zinc-800 pb-2">
                                            <label className="text-[10px] font-medium text-zinc-200 sm:text-[11px]">
                                                Before-due presets
                                            </label>
                                            <div className="flex max-h-28 flex-wrap gap-1 overflow-y-auto [scrollbar-width:thin]">
                                                {reminderLabelOptions.map((label) => {
                                                    ...
                                                    return (
                                                        <label
                                                            key={label.labelName}
                                                            className="flex cursor-pointer items-center gap-0.5 rounded border border-zinc-700/80 bg-zinc-950/80 px-1 py-0.5 text-[10px] text-zinc-200 sm:text-[11px]"
                                                        >
                                                            <input
                                                                type="checkbox"
                                        ...
                                        <div className="space-y-0.5">
                                            <label className="text-[10px] text-zinc-400 sm:text-[11px]">
                                                Exact time
                                            </label>
                                            <div className="flex flex-wrap items-center gap-1">
                                                <input
                                                    type="datetime-local"
                                                    className="min-w-0 flex-1 rounded border border-zinc-700 bg-zinc-900 py-0.5 px-1 text-[10px] text-zinc-100 sm:py-1 sm:text-xs"
                                                    value={exactLocalDue}
                                                    onChange={(e) => setExactLocalDue(e.target.value)}
                                                />
                                                <button
                                                    type="button"
                                                    className="inline-flex shrink-0 items-center gap-0.5 rounded border border-indigo-700 bg-indigo-600 px-1.5 py-0.5 text-[10px] font-medium text-white hover:bg-indigo-9500 sm:text-[11px]"
                                                    onClick={addExactDue}
                                                >
                                        ...
                                        <div className="space-y-0.5">
                                            <label className="text-[10px] text-zinc-400 sm:text-[11px]">Cron (UTC)</label>
                                            <div className="flex flex-wrap items-center gap-1">
                                                <input
                                                    type="text"
                                                    className="min-w-0 flex-1 rounded border border-zinc-700 bg-zinc-900 py-0.5 px-1 font-mono text-[10px] text-zinc-100 sm:py-1 sm:text-[11px]"
                                                    placeholder="0 9 * * *"
                                                    value={cronInputDue}
                        ...
                        <ReadOnlySendQueue
                            ...
                            pendingBorderClass="border-zinc-700/80 bg-zinc-950/90"
                            completedBorderClass="border-zinc-700/80 bg-zinc-800/80"
                        />
                    </div>
                )}
            </section>
            ...
            >
                ...
                {expandedRemainder && (
                    <div className="border-t border-zinc-800 px-2 pb-2 pt-1 sm:px-2.5 sm:pb-2.5">
                        <p className="mb-2 text-[10px] leading-snug text-zinc-400 sm:text-[11px]">
                        ...
                        <div className="grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-2">
                            <div className="min-w-0 rounded-md border border-zinc-700/80 bg-zinc-900/90 p-1.5 sm:p-2">
                                ...
                                <div className="max-h-40 space-y-1 overflow-y-auto [scrollbar-width:thin]">
                                    ...
                                    {remainderAbsoluteTimesIso.map((iso) => (
                                        <div
                                            key={iso}
                                            className="flex items-start justify-between gap-1 rounded border border-indigo-800 bg-indigo-950/70 px-1.5 py-1 text-[10px] sm:text-[11px]"
                                        >
                                            <div className="min-w-0">
                                                <span className="font-medium text-indigo-200">Exact time</span>
                                                <span className="flex items-center gap-0.5 text-zinc-700">
                                                    <LucideClock className="h-3 w-3 shrink-0" strokeWidth={2} />
                                                    {fmtWhen(iso)}
                                                </span>
                            ...
                            <div className="min-w-0 space-y-1.5 rounded-md border border-zinc-700/80 bg-zinc-900/90 p-1.5 sm:p-2">
                                <p className="text-[10px] font-medium uppercase tracking-wide text-zinc-500 sm:text-[11px]">
                                    Add
                                </p>
                                <div className="space-y-0.5">
                                    <label className="text-[10px] text-zinc-400 sm:text-[11px]">Exact time</label>
                                    <div className="flex flex-wrap items-center gap-1">
                                        <input
                                            type="datetime-local"
                                            className="min-w-0 flex-1 rounded border border-zinc-700 bg-zinc-900 py-0.5 px-1 text-[10px] text-zinc-100 sm:py-1 sm:text-xs"
