...
const TaskAddOrEdit: React.FC<{
    ...
    const currentWorkspaceId = useAtomValue(jotaiStateTaskWorkspaceId);
    ...
    useEffect(() => {
        const fetchTaskById = async () => {
            if (isTaskAddModalIsOpen.modalType === 'edit' && isTaskAddModalIsOpen.recordId) {
                ...
                // Prefer the currently selected workspace when adding a task
                if (currentWorkspaceId.length === 24) {
    ...
    const handleDuePreset = (days: number) => {
        const base = new Date();
    ...
    return (
        <div>
            {isTaskAddModalIsOpen.openStatus && (
                <div
                    className="fixed inset-0 z-[1000] flex items-end justify-center bg-zinc-950/40 p-0 backdrop-blur-sm sm:items-center sm:p-3"
                    onKeyDown={handleModalKeyDown}
                    role="dialog"
                    aria-modal="true"
                    aria-label={isTaskAddModalIsOpen.modalType === "add" ? "Add task dialog" : "Edit task dialog"}
                >
                    <div className="custom-scrollbar mx-auto w-full max-w-3xl sm:max-h-[90vh]">
                        <div className="relative w-full overflow-hidden rounded-t-xl border border-zinc-700/60 bg-zinc-900 shadow-xl sm:rounded-xl">
                            <div className="custom-scrollbar max-h-[90vh] overflow-y-auto bg-zinc-950/40 p-2 sm:max-h-[85vh] sm:p-4 [scrollbar-width:thin] [scrollbar-color:rgb(113_113_122)_rgb(39_39_42)]">
                                <h2 className="mb-1 text-center text-sm font-semibold tracking-tight text-zinc-100 sm:mb-3 sm:text-base">
                                ...
                                <div className="py-0.5 sm:py-1">
                                    ...
                                    <div className='mt-0.5 flex flex-wrap items-center gap-0.5 sm:gap-1'>
                                        <button
                                            ...
                                            className="rounded-lg border border-zinc-700/80 bg-zinc-900 px-1.5 py-0.5 text-[11px] font-medium text-zinc-200 shadow-sm transition-colors hover:bg-zinc-800 sm:px-2 sm:py-1"
                                        >
                                            Today
                                        </button>
                                        <button
                                    ...
                                    {taskAiSuggestion.display && (
                                        <div className='mt-0.5 sm:mt-1'>
                                            <div className="mt-0.5 rounded-xl border border-zinc-700/60 bg-zinc-900 p-1.5 shadow-sm sm:mt-1 sm:p-2">
                                                <span className="text-[11px] font-medium text-zinc-400">AI</span>
                                                <textarea
                                                ...
                                                <button
                                                    type="button"
                                                    className="mt-0.5 rounded-lg bg-indigo-600 px-1.5 py-0.5 text-[11px] font-medium text-white shadow-sm transition-colors hover:bg-indigo-9500 sm:mt-1 sm:px-2 sm:py-1"
                                                    onClick={() => {
                                ...
                                {isTaskAddModalIsOpen.modalType === 'edit' && (
                                    <div className="mb-1.5 flex flex-wrap gap-0.5 sm:mb-2 sm:gap-1">
                                        <button
                                            ...
                                            className="rounded-lg bg-indigo-600 px-1.5 py-0.5 text-[11px] font-medium text-white shadow-sm transition-colors hover:bg-indigo-9500 disabled:opacity-60 sm:px-2 sm:py-1"
                                            disabled={taskAiSuggestionLoading}
                                        >
                                            {taskAiSuggestionLoading ? 'Loading…' : 'AI suggest'}
                                        </button>
                                        <span className="rounded-md border border-zinc-700/80 bg-zinc-800/80 px-1.5 py-0.5 text-[11px] font-medium text-zinc-200">
                                            Status: {status}
                                        </span>
                                        {formData.isCompleted && (
                                            <span className="rounded-md border border-emerald-700/80 bg-emerald-950/90 px-1.5 py-0.5 text-[11px] font-semibold text-emerald-200">
                                                Completed
                                            </span>
                                        )}
                                        {formData.isArchived && (
                                            <span className="rounded-md border border-zinc-700/80 bg-zinc-800 px-1.5 py-0.5 text-[11px] font-semibold text-zinc-300">
                                                Archived
                                            </span>
                                        )}
                                        {formData.priority && (
                                            <span className="rounded-md border border-zinc-700/80 bg-zinc-950 px-1.5 py-0.5 text-[11px] font-medium text-zinc-200">
                                        ...
                                        {labels.map((label, index) => (
                                            <span
                                                key={index}
                                                className="mr-0.5 inline-flex items-center gap-0.5 rounded-md border border-zinc-700/70 bg-zinc-950 px-1.5 py-0.5 text-[11px] font-medium text-zinc-200"
                                            >
                                                <span>{label}</span>
                                                <button
                                                    type="button"
                                                    className="inline-flex rounded-md p-0.5 text-zinc-500 transition hover:bg-zinc-800 hover:text-zinc-200"
                                                    onClick={() => {
                                        ...
                                        {taskAiSuggestion.newTaskTags.map((label, index) => (
                                            <button
                                                key={index}
                                                type="button"
                                                className="mr-0.5 rounded-md border border-zinc-700/80 bg-zinc-900 px-1.5 py-0.5 text-[11px] font-medium text-zinc-200 shadow-sm transition hover:border-zinc-500 hover:bg-zinc-800"
                                                onClick={() => {
                                ...
                                <div className="mt-0.5 flex flex-wrap gap-0.5 sm:mt-1 sm:gap-1">
                                    {isTaskAddModalIsOpen.modalType === 'edit' && (
                                        <Fragment>
                                            <label className="flex cursor-pointer items-center gap-1 rounded-lg border border-zinc-700/80 bg-zinc-900 px-1.5 py-0.5 shadow-sm transition hover:border-zinc-500 sm:gap-1.5 sm:px-2 sm:py-1">
                                                <input
                                                    type="checkbox"
                                            ...
                                            <label className="flex cursor-pointer items-center gap-1 rounded-lg border border-zinc-700/80 bg-zinc-900 px-1.5 py-0.5 shadow-sm transition hover:border-zinc-500 sm:gap-1.5 sm:px-2 sm:py-1">
                                                ...
                                                />
                                                <span className="text-xs font-medium text-zinc-200">Archived</span>
                                            </label>

                                            {/* priority */}
                                            <div className="flex flex-wrap items-center gap-1 rounded-lg border border-zinc-700/70 bg-zinc-950/80 px-1.5 py-1 shadow-sm sm:gap-1.5 sm:px-2 sm:py-1.5">
                                    ...
                                    {/* workspace */}
                                    <div className="flex min-w-0 flex-[1_1_12rem] flex-wrap items-center gap-1 rounded-lg border border-zinc-700/70 bg-zinc-900 px-1.5 py-1 shadow-sm sm:gap-1.5 sm:px-2 sm:py-1.5">
                                        <label className="shrink-0 text-xs font-medium text-zinc-300">Workspace</label>
                                    ...
                                    {/* status */}
                                    <div className="flex min-w-0 flex-[1_1_12rem] flex-wrap items-center gap-1 rounded-lg border border-zinc-700/70 bg-zinc-900 px-1.5 py-1 shadow-sm sm:gap-1.5 sm:px-2 sm:py-1.5">
                                        <label className="shrink-0 text-xs font-medium text-zinc-300">List status</label>
                                    ...
                                    {isTaskAddModalIsOpen.modalType === 'add' && (
                                        <div className="flex min-w-0 flex-wrap items-center gap-1 rounded-lg border border-zinc-700/70 bg-zinc-950/80 px-1.5 py-1 shadow-sm sm:gap-1.5 sm:px-2 sm:py-1.5">
                                            <label className="shrink-0 text-xs font-medium text-zinc-300">Due date</label>
                                            <input
                                                type="datetime-local"
                                                className="min-w-0 flex-1 rounded-lg border border-zinc-700/90 bg-zinc-900 py-0.5 px-1 text-xs text-zinc-100 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 sm:py-1 sm:px-1.5"
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => handleDuePreset(1)}
                                                className="shrink-0 rounded-lg border border-zinc-700 bg-zinc-900 px-1.5 py-0.5 text-[11px] font-medium text-zinc-200 hover:bg-zinc-800"
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => handleDuePreset(7)}
                                                className="shrink-0 rounded-lg border border-zinc-700 bg-zinc-900 px-1.5 py-0.5 text-[11px] font-medium text-zinc-200 hover:bg-zinc-800"
                                                aria-label="Set due date to 7 days from now"
                                            >
                                                +7d
                                            </button>
                                            {dueDate && (
                                    ...
                                    {isTaskAddModalIsOpen.modalType === 'edit' && (
                                        <div className="flex flex-wrap items-center gap-1 rounded-lg border border-zinc-700/70 bg-zinc-950/80 px-2 py-1.5">
                                            <span className="text-xs font-medium text-zinc-300">Due presets</span>
                                            <button
                                                type="button"
                                                onClick={() => handleDuePreset(1)}
                                                className="rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1 text-[11px] font-medium text-zinc-200 hover:bg-zinc-800"
                                                aria-label="Set due date to tomorrow"
                                            >
                                                +1d
                                            </button>
                                ...
                                <div className="py-0.5 sm:py-1">
                                    <label className="mb-0.5 block text-xs font-medium text-zinc-400 sm:mb-1">Description</label>
                                    <textarea
                                    ...
                                    {taskAiSuggestion.display && (
                                        <div className="mt-1 sm:mt-1.5">
                                            <div className="rounded-xl border border-zinc-700/60 bg-zinc-900 p-1.5 shadow-sm sm:p-2">
                                                <span className="text-[11px] font-medium text-zinc-400">AI suggestion</span>
                                                <textarea
                            ...
                            <div className="flex justify-end gap-1 border-t border-zinc-800 bg-zinc-900 px-2 py-2 sm:gap-1.5 sm:px-4 sm:py-2.5">
                                <button
                                    type="button"
                                    onClick={toggleModal}
                                    className="rounded-lg border border-zinc-700/80 bg-zinc-900 px-2 py-1 text-xs font-medium text-zinc-300 shadow-sm transition-colors hover:bg-zinc-800 sm:px-3 sm:py-1.5"
                                    aria-label="Cancel"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className="rounded-lg bg-indigo-600 px-2 py-1 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-indigo-9500 sm:px-3 sm:py-1.5"
                                    aria-label={isTaskAddModalIsOpen.modalType === "add" ? "Add task" : "Save task"}
                                    title="Ctrl+Enter to save"
                                >
                                    {isTaskAddModalIsOpen.modalType === 'add' ? 'Add task' : 'Save'}
                                    <span className="ml-1 hidden text-[10px] opacity-70 sm:inline">(Ctrl+Enter)</span>
                                </button>
                            </div>
                        </div>
                    </div>
