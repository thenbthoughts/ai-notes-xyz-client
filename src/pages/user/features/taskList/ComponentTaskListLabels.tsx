    ...
    return (
        <div>
            ...
            <input
                ...
                className="mb-1.5 w-full rounded-lg border border-zinc-700/90 bg-zinc-950/80 py-1.5 px-2 text-xs text-zinc-100 shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15"
                value={searchInput}
                onChange={(e) => setSearchInput(e.target.value)}
            />

            {selectedLabels.length > 0 && (
                <div className="mb-1.5 rounded-lg border border-zinc-700/70 bg-zinc-950/80 p-1.5 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-0.5">
                        {selectedLabels.map((label) => (
                            <span
                                key={label}
                                className="inline-flex items-center gap-0.5 rounded-md border border-zinc-700/80 bg-zinc-900 px-1.5 py-0.5 text-[10px] font-medium leading-tight text-zinc-200 shadow-sm"
                            >
                                {label}
                                <button
                                    type="button"
                        ...
                        {filteredLabels.length === 0 ? (
                            ...
                            <>
                                        ...
                                        <button
                                            ...
                                            className={
                                                (on
                                                    ...
                                                    : 'border-zinc-700/80 bg-zinc-900 text-zinc-200 shadow-sm hover:border-zinc-500 hover:bg-zinc-800 ') +
                                                'inline-flex items-center gap-0.5 rounded-md border px-1.5 py-0.5 text-[10px] font-medium leading-tight transition-colors'
                                            }
                                        >
                                            {label._id}
                                            <span className={on ? 'rounded bg-zinc-900/20 px-0.5 text-[9px] tabular-nums' : 'rounded bg-zinc-700/80 px-0.5 text-[9px] tabular-nums text-zinc-300'}>
                                                {label.count}
                                            </span>
                                        </button>
                                    );
                                ...
                                {filteredLabels.length > 100 && (
                                    <span className="rounded border border-amber-700/80 bg-amber-950 px-1 py-px text-[10px] text-amber-200">
                                        +{filteredLabels.length - 100}
                                    </span>
                                )}
                            </>
