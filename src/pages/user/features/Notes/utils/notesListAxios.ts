...
export const notesBulkMoveAxios = async ({
    noteIds,
    targetWorkspaceId,
    targetFolder,
    orderMap,
    ...
    try {
        const response = await axiosCustom.post('/api/notes/crud/notesBulkMove', {
            noteIds,
            targetWorkspaceId,
            targetFolder,
            orderMap,
        });
        return { success: response.data.message || 'Moved', error: '' };
    } catch (error) {
        console.error(error);
        return { success: '', error: 'Bulk move failed' };
    }
};

export const notesVersionsGetAxios = async ({
    noteId,
}: {
    noteId: string;
}) => {
    try {
        const response = await axiosCustom.get('/api/notes/crud/notesVersions', {
            params: { noteId },
        });
        return { docs: response.data.docs || [], error: '' };
    } catch (error) {
        console.error(error);
        return { docs: [], error: 'Failed to fetch versions' };
    }
};
