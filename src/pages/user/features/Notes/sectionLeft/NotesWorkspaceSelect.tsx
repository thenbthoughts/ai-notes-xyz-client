...
const notesChromeSelectClass =
    'w-full rounded-lg border border-zinc-700/90 bg-zinc-900 py-2 pl-3 pr-2 text-[13px] font-normal text-zinc-100 shadow-sm transition-shadow focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 disabled:cursor-not-allowed disabled:opacity-50';
...
export function NotesWorkspaceSelect({ selectedId, onSelect }: NotesWorkspaceSelectProps) {
    ...
    if (loading) {
        return (
            <div className="rounded-lg border border-dashed border-zinc-700/80 bg-zinc-950 px-2 py-3 text-center font-mono text-[10px] text-zinc-400">
                Loading workspaces…
            </div>
        );
    }
    ...
    if (workspaces.length === 0) {
        return (
            <div className="rounded-lg border border-zinc-700/80 bg-zinc-900 px-2 py-2 text-[11px] text-zinc-400 shadow-sm">
                No workspaces.{' '}
