...
const ComponentChatHistory = () => {
    ...
    const presetBtn =
        'rounded-lg border border-zinc-700/80 bg-zinc-900 px-1.5 py-0.5 text-[10px] font-medium text-zinc-300 shadow-sm hover:bg-zinc-800';

    const renderDateRange = () => (
        <div className="mb-2 rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-2 sm:p-2.5">
            <div className="mb-1.5 text-[10px] font-medium text-zinc-400">Date range</div>
            <input
                type="date"
                className="mb-1.5 block w-full rounded-lg border border-zinc-700/90 bg-zinc-900 px-2 py-1.5 text-xs text-zinc-100 shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15"
                value={dateRange.startDate ? dateRange.startDate.toISOString().split('T')[0] : ''}
                onChange={(e) => setDateRange({ ...dateRange, startDate: new Date(e.target.value) })}
            />
            <input
                type="date"
                className="mb-2 block w-full rounded-lg border border-zinc-700/90 bg-zinc-900 px-2 py-1.5 text-xs text-zinc-100 shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15"
    ...
    return (
        <div className="px-2 py-2 text-zinc-100 sm:px-2.5 sm:py-2.5">
            <div className="mb-2 flex items-baseline justify-between gap-2">
                <h2 className="text-xs font-semibold tracking-tight text-zinc-100 sm:text-sm">Life events</h2>
            ...
            <div className="mb-2 flex flex-col gap-1.5 sm:flex-row sm:flex-wrap sm:gap-2">
                <button
                    type="button"
                    className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-indigo-600/20 bg-indigo-600 py-2 text-xs font-medium text-white shadow-sm hover:bg-indigo-9500 sm:min-w-0 sm:py-1.5"
                ...
                <button
                    type="button"
                    className="rounded-lg border border-zinc-700/80 bg-zinc-900 px-3 py-2 text-xs font-medium text-zinc-300 shadow-sm hover:bg-zinc-800 sm:py-1.5"
                ...
                <Link
                    ...
                    className="inline-flex flex-1 items-center justify-center gap-1 rounded-lg border border-zinc-700/80 bg-zinc-900 py-2 text-xs font-medium text-zinc-200 shadow-sm hover:bg-zinc-800 sm:py-1.5"
                >
            ...
            <div className="relative mb-2">
                ...
                <DebounceInput
                    ...
                    type="text"
                    placeholder="Search…"
                    className="w-full rounded-lg border border-zinc-700/90 bg-zinc-900 py-2 pl-8 pr-2 text-xs text-zinc-100 shadow-sm placeholder:text-zinc-400 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15 sm:py-1.5"
                    onChange={(e) => setSearchTerm(e.target.value)}
                    value={searchTerm}
                />
            </div>

            <div className="mb-2 space-y-2 [&_label]:text-xs [&_label]:font-medium [&_label]:text-zinc-300 [&_select]:text-xs">
            ...
            <div className="mb-2 rounded-xl border border-zinc-700/80 bg-zinc-950/90 px-2 py-1.5 sm:px-2.5 sm:py-2">
                <div className="mb-1.5 text-[10px] font-medium text-zinc-400">Starred</div>
                <div className="flex flex-wrap gap-1">
                    {[
                        ...
                        <button
                            ...
                            className={`rounded-lg border px-2 py-1 text-[11px] font-medium shadow-sm transition-colors sm:py-0.5 ${
                                isStar === opt.value
                                    ...
                                    : 'border-zinc-700/80 bg-zinc-900 text-zinc-300 hover:border-zinc-500 hover:bg-zinc-800'
                            }`}
                        >
                            {opt.label}
                        </button>
            ...
            <div className="mb-2 rounded-xl border border-zinc-700/80 bg-zinc-900 px-2 py-1.5 shadow-sm sm:px-2.5 sm:py-2">
                <div className="mb-1.5 text-[10px] font-medium text-zinc-400">Impact</div>
                <div className="flex flex-wrap gap-1">
                    {[
                        { label: 'All', value: '' as const },
                        ...
                        <button
                            ...
                            className={`rounded-lg border px-1.5 py-1 text-[10px] font-medium shadow-sm sm:py-0.5 ${
                                impact === opt.value
                                    ? 'border-indigo-600 bg-indigo-600 text-white'
                                    : 'border-zinc-700/80 bg-zinc-950/80 text-zinc-300 hover:bg-zinc-800'
                            }`}
                        >
                            {opt.label}
                        </button>
            ...
            <div className="flex items-center gap-2 rounded-xl border border-zinc-700/80 bg-zinc-900 px-2 py-2 shadow-sm sm:py-1.5">
                <input
                    type="checkbox"
                    id="hideDailyDiary"
                ...
                />
                <label htmlFor="hideDailyDiary" className="cursor-pointer text-xs text-zinc-300">
                    Hide daily diary
                </label>
            </div>
        </div>
...
const ComponentChatHistoryRender = () => {
    return (
        <div className="border-r border-zinc-700/80 bg-zinc-800/50">
            <div className="h-[calc(100vh-60px)] overflow-y-auto overflow-x-hidden bg-zinc-900 [scrollbar-width:thin]">
...
const ComponentChatHistoryModelRender = () => {
    return (
        <div className="fixed left-0 top-[60px] z-[1001] w-[min(320px,calc(100vw-50px))] border-r border-zinc-700/80 bg-zinc-900 shadow-xl">
