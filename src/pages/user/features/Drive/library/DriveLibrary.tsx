...
const DriveLibrary = () => {
    ...
    return (
        <>
            ...
            <div className="mb-3 rounded-2xl border border-zinc-700 bg-zinc-900 p-3 shadow-sm sm:p-4">
            ...
            {!loading && !loadError && (
                <div className="mb-2 px-1 text-sm text-zinc-400">
                    {totalCount > 0 ? (
                        <>
                    ...
                    viewMode === 'grid' ? (
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                            {Array.from({ length: 12 }).map((_, i) => (
                                <div
                                    key={i}
                                    className="h-36 animate-pulse rounded-xl bg-zinc-800/80"
                                />
                            ))}
                        </div>
                    ) : (
                        <div className="space-y-2">
                            {Array.from({ length: 10 }).map((_, i) => (
                                <div
                                    key={i}
                                    className="h-12 animate-pulse rounded-lg bg-zinc-800/80"
                                />
                            ))}
                        </div>
                    )
            ...
            {!loading && !loadError && totalPages > 1 && (
                <div className="mt-4 flex justify-center">
                    <ReactPaginate
                        ...
                        pageClassName="rounded-lg border border-zinc-700 bg-zinc-900 hover:bg-zinc-800 text-sm"
                        previousClassName="rounded-lg border border-zinc-700 bg-zinc-900 hover:bg-zinc-800 text-sm"
                        previousLinkClassName="block px-3 py-1.5 text-zinc-300"
                        nextClassName="rounded-lg border border-zinc-700 bg-zinc-900 hover:bg-zinc-800 text-sm"
                        nextLinkClassName="block px-3 py-1.5 text-zinc-300"
                        breakClassName="rounded-lg border border-zinc-700 bg-zinc-900 text-sm"
                        breakLinkClassName="block px-3 py-1.5 text-zinc-300"
                        activeLinkClassName="!bg-sky-600 !text-white !border-sky-600"
                        pageLinkClassName="block px-3 py-1.5 text-zinc-300"
                    />
                </div>
            )}
