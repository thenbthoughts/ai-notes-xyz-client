...
const DriveMarkdownEditor = ({ file, bucketName, onClose, onSave }: DriveMarkdownEditorProps) => {
    ...
    if (loading) {
        return (
            <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-black bg-opacity-75">
                <div className="bg-zinc-900 text-zinc-100 rounded-lg p-8">
                    <p>Loading file...</p>
                </div>
            </div>
        );
    ...
    return (
        ...
        >
            <div
                className="bg-zinc-900 text-zinc-100 rounded-lg w-full max-w-6xl h-[90vh] flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="flex items-center justify-between p-4 border-b border-zinc-700">
                    <h2 className="text-lg font-semibold">{file.fileName}</h2>
                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1 border border-zinc-700 rounded-sm">
                            <button
                                onClick={() => setViewMode('edit')}
                                className={`px-3 py-1 ${viewMode === 'edit' ? 'bg-blue-600 text-white' : 'bg-zinc-900 text-zinc-100'} transition`}
                            >
                                <LucideEdit size={16} />
                            </button>
                            <button
                                onClick={() => setViewMode('split')}
                                className={`px-3 py-1 ${viewMode === 'split' ? 'bg-blue-600 text-white' : 'bg-zinc-900 text-zinc-100'} transition`}
                            >
                                Split
                            </button>
                            <button
                                onClick={() => setViewMode('preview')}
                                className={`px-3 py-1 ${viewMode === 'preview' ? 'bg-blue-600 text-white' : 'bg-zinc-900 text-zinc-100'} transition`}
                            >
                                <LucideEye size={16} />
                            </button>
                        </div>
                        ...
                        <button
                            onClick={onClose}
                            className="p-2 hover:bg-zinc-800 rounded-sm transition text-zinc-100"
                        >
                            <LucideX size={24} />
                        </button>
                    </div>
                ...
                <div className="flex-1 flex overflow-hidden">
                    {(viewMode === 'edit' || viewMode === 'split') && (
                        <div className={viewMode === 'split' ? 'w-1/2 border-r border-zinc-700' : 'w-full'}>
                            <textarea
                                value={content}
                                onChange={(e) => setContent(e.target.value)}
                                className="w-full h-full p-4 border-0 resize-none focus:outline-none font-mono text-sm bg-zinc-900 text-zinc-100"
