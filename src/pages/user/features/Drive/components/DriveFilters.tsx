            ...
            <div className="flex flex-wrap items-center gap-2">
                <div className="relative min-w-[200px] flex-1">
                    ...
                    <DebounceInput
                        ...
                        className="w-full rounded-full border border-zinc-700 bg-zinc-800 py-2.5 pl-9 pr-9 text-sm text-zinc-200 outline-none transition focus:border-sky-300 focus:bg-zinc-900 focus:ring-2 focus:ring-sky-100"
                    />
                    {searchQuery && (
                        <button
                            type="button"
                ...
                <div className="flex items-center gap-1 rounded-full border border-zinc-700 bg-zinc-900 px-2 py-1">
                    <LucideArrowUpDown size={14} className="text-zinc-500" />
                    <select
                        value={sortBy}
                        onChange={(e) => onSortByChange(e.target.value as 'name' | 'size' | 'date')}
                        className="border-0 bg-transparent py-1 text-sm text-zinc-300 outline-none"
                        aria-label="Sort by"
                    >
                ...
                <button
                    ...
                    className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-2 text-sm transition ${
                        showFilters || fileTypeFilter.length > 0
                            ? 'border-sky-300 bg-sky-950 text-sky-300'
