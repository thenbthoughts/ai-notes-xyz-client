...
export function useMemoNotes() {
  ...
  const updateNote = useCallback(
    async (
      id: string,
      patch: Partial<Pick<MemoNote, 'title' | 'body' | 'labelIds' | 'pinned' | 'archived' | 'trashed' | 'noteColor' | 'sortOrder' | 'reminderTime'>> & { reminderTime?: number | string | null },
    ) => {
      try {
        await memoEdit(id, patch as unknown as Parameters<typeof memoEdit>[1]);
        setNotes((prev) =>
          prev.map((n) => {
            if (n.id !== id) return n;
  ...
  const togglePin = useCallback(
    async (id: string) => {
      const current = notesRef.current.find((x) => x.id === id);
      if (!current) return;
      const nextPinned = !current.pinned;
      setNotes((prev) =>
        prev.map((x) => (x.id === id ? { ...x, pinned: nextPinned, updatedAt: Date.now() } : x)),
      );
      try {
  ...
  const reorderNote = useCallback(
    async (id: string, toIndex: number, orderedIds: string[]) => {
      const i = orderedIds.indexOf(id);
      if (i < 0) return;
      const j = Math.max(0, Math.min(orderedIds.length - 1, toIndex));
      if (j === i) return;

      const nextIds = [...orderedIds];
      const [moved] = nextIds.splice(i, 1);
      if (!moved) return;
      nextIds.splice(j, 0, moved);

      const base = Date.now();
      const orderById = new Map<string, number>();
      for (let k = 0; k < nextIds.length; k++) {
        orderById.set(nextIds[k]!, base + (nextIds.length - k));
      }

      setNotes((prev) =>
        prev.map((x) => {
          const sortOrder = orderById.get(x.id);
          return sortOrder === undefined ? x : { ...x, sortOrder };
        }),
      );

      try {
        await memoRevalidateSortOrderById(id, { jumpToPosition: j + 1 });
        const list = await fetchMemoList();
        setNotes(list);
      } catch (e) {
        console.error(e);
        toast.error('Could not reorder memo');
        void refresh();
      }
    },
    [refresh],
  ...
  const setReminder = useCallback(async (id: string, iso: string | null) => {
    const ts = iso ? new Date(iso).getTime() : null;
    if (iso && (ts === null || Number.isNaN(ts))) {
      toast.error('Invalid reminder time');
      return;
    }
    try {
      await memoSetReminder(id, iso);
      setNotes((prev) => prev.map((n) => (n.id === id ? { ...n, reminderTime: ts, updatedAt: Date.now() } : n)));
  ...
  const bulkAction = useCallback(async (ids: string[], action: string) => {
    ...
    try {
      await memoBulkAction(ids, action);
      await refresh();
