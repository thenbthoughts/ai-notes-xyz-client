...
function mapDoc(d: ApiMemoDoc): MemoNote {
  ...
  return {
    ...
    reminderTime: d.reminderTime ? new Date(d.reminderTime).getTime() : null,
...
export async function memoSetReminder(id: string, reminderTime: string | null): Promise<void> {
  await axiosCustom.post(`${MEMO_BASE}/memoReminders`, { _id: id, reminderTime });
}

export async function memoBulkAction(ids: string[], action: string): Promise<void> {
  await axiosCustom.post(`${MEMO_BASE}/memoBulkAction`, { ids, action });
}

/** Reorder memo by jumpToPosition (1-based), then revalidate pinned and unpinned sort groups. */
export async function memoRevalidateSortOrderById(
  id: string,
  opts: { jumpToPosition: number },
): Promise<void> {
  await axiosCustom.post(`${MEMO_BASE}/memoRevalidateSortOrderById`, { _id: id, ...opts });
}
