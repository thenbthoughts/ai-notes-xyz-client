...
export default function MemoSidebar({
  ...
  const activeCls = 'bg-[#422006] hover:bg-[#422006]';

  const itemBase = collapsed
    ? 'flex min-h-10 w-full items-center justify-center rounded-lg px-1.5 text-xs font-medium text-[#e4e4e7] transition hover:bg-[#18181b] active:bg-[#3f3f46]'
    : 'flex min-h-10 w-full items-center gap-2.5 rounded-r-full py-1.5 pl-3 pr-2 text-xs font-medium text-[#e4e4e7] transition hover:bg-[#18181b] active:bg-[#3f3f46] sm:gap-3 sm:pl-4 sm:pr-3';
  ...
  const Item = ({
    ...
    return (
      ...
      >
        <Icon className="h-4 w-4 shrink-0 text-[#a1a1aa]" strokeWidth={2} />
        {!collapsed && (
          <>
  ...
  return (
    <aside className="relative flex h-full min-h-0 w-full flex-col bg-[#27272a]">
      <nav className="flex min-h-0 flex-1 flex-col gap-0 overflow-y-auto overscroll-y-contain px-1 py-1 sm:px-0">
        ...
        {labels.map((l) => {
          ...
          return (
            <Item
              ...
              trailing={
                count > 0 ? (
                  <span className="rounded-full bg-[#3f3f46] px-1.5 py-0 text-[10px] font-medium tabular-nums text-[#a1a1aa]">
                    {count}
                  </span>
                ) : undefined
              }
        ...
        <button
          ...
          style={{ fontFamily: 'Roboto, system-ui, sans-serif' }}
        >
          <LucideEdit3 className="h-4 w-4 shrink-0 text-[#a1a1aa]" strokeWidth={2} />
