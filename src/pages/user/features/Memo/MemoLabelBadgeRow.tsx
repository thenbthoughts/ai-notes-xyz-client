    ...
    <div className={`flex flex-wrap gap-1 ${className}`} aria-label="Labels">
      {note.labelIds.map((lid, i) => {
        ...
        return (
          <span
            key={lid}
            className="inline-flex max-w-full shrink-0 items-center truncate rounded-full border border-[#52525b]/70 bg-[#3f3f46] px-2 py-px text-[10px] font-medium text-[#e4e4e7]"
