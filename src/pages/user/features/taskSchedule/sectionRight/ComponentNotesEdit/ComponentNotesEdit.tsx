...
import { LucideArrowLeft, LucidePlus, LucideSave, LucideTrash, LucidePlay, LucideHistory, LucideEye, LucideCopy } from 'lucide-react';
...
const getLogStatusClassEdit = (status: string): string => {
    if (status === 'success') {
...
const cronExplain = (expr: string): string => {
    const t = expr.trim();
    if (t === '0 9 * * *') { return 'Daily at 09:00'; }
    if (t === '0 8 * * *') { return 'Daily at 08:00'; }
    if (t === '0 9 * * 1-5') { return 'Weekdays at 09:00'; }
    if (t.startsWith('0 */')) { return `Every ${t.split(' ')[1].replace('*/', '')}h`; }
    if (t === '0 0 * * 0') { return 'Sundays at midnight'; }
    return t;
};

const getDraftKey = (id: string): string => `ts_draft_${id}`;
const loadDraft = (id: string): Record<string, unknown> | null => {
    try {
...
const saveDraft = (id: string, data: Record<string, unknown>) => {
    try {
...
const ScheduleTimeArr = ({
    ...
    return (
        <div>
            <label className="mb-2 block text-sm font-medium text-zinc-200 sm:mb-3">Schedule Times</label>

            {/* Display existing schedule times */}
            <div className="mb-3 flex flex-wrap gap-1.5 sm:mb-4 sm:gap-2">
                {scheduleTimeArr.map((scheduleTime, idx) => (
                    <span key={idx} className="inline-flex max-w-full items-center gap-0.5 rounded-lg border border-indigo-700/70 bg-indigo-950/90 px-2 py-0.5 text-xs font-medium text-indigo-200 shadow-sm sm:px-2.5 sm:py-1">
            ...
            <div className="space-y-3 rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-3 sm:space-y-4 sm:p-4">
                {/* Date and Time */}
                <div>
                    ...
                    <input
                        ...
                        className="w-full rounded-lg border border-zinc-700 bg-zinc-900 p-2 text-sm text-zinc-100 shadow-sm transition focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15"
                ...
                <button
                    type="button"
                    className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-9500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 sm:py-2.5"
                    onClick={() => {
...
const CronExpressionArr = ({
    ...
    return (
        <div>
            <label className="mb-2 block text-sm font-medium text-zinc-200 sm:mb-3">Cron Expressions</label>

            {/* Display existing cron expressions */}
            <div className="mb-3 flex flex-wrap gap-1.5 sm:mb-4 sm:gap-2">
                {cronExpressionArr.map((cronExpr, idx) => (
                    <span key={idx} className="inline-flex max-w-full items-center gap-0.5 rounded-lg border border-zinc-700/80 bg-zinc-900 px-2 py-0.5 font-mono text-xs font-medium text-zinc-200 shadow-sm sm:px-2.5 sm:py-1">
            ...
            <div className="space-y-3 rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-3 sm:space-y-4 sm:p-4">
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch">
                        <input
                            ...
                            className="min-w-0 flex-1 rounded-lg border border-zinc-700 bg-zinc-900 p-2 text-sm text-zinc-100 shadow-sm transition focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/15"
                            onChange={e => setCronExpressionInput(e.target.value)}
                            placeholder="Enter cron expression (e.g., 0 9 * * 1-5)"
                        />
                        <button
                            type="button"
                            className="w-full shrink-0 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-9500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 sm:w-auto"
                            onClick={() => {
                ...
                <div className="flex items-center">
                    <div className="flex-1 border-t border-zinc-700/80"></div>
                    <div className="bg-zinc-950/90 px-2 text-xs text-zinc-500 sm:px-3 sm:text-sm">OR</div>
                    <div className="flex-1 border-t border-zinc-700/80"></div>
                </div>
                ...
                <button
                    type="button"
                    className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-indigo-700 shadow-sm ring-1 ring-inset ring-zinc-200/80 transition hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/25"
                ...
                {showCronBuilder && (
                    <div className="rounded-xl border border-zinc-700/80 bg-zinc-900 p-3 sm:p-4">
                        <h4 className="mb-3 text-sm font-semibold text-zinc-100 sm:mb-4 sm:text-base">Dynamic Cron Expression Builder</h4>
                                ...
                                {['hour', 'daily', 'weekly', 'monthly'].map((type) => (
                                    <button
                                        ...
                                        className={`rounded-lg border px-2 py-2 text-xs font-medium sm:px-3 sm:text-sm ${cronBuilder.type === type
                                            ...
                                            : 'border-zinc-700 bg-zinc-900 text-zinc-700 hover:bg-zinc-800'
                                            }`}
                                    >
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <input
                                            ...
                                            className="w-20 rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 sm:px-3 sm:py-2"
                                        />
                                        <span className="text-sm text-zinc-400">hour(s)</span>
                                    </div>
                                </div>
                            )}
                            ...
                            {cronBuilder.type === 'daily' && (
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 sm:px-3 sm:py-2"
                                    />
                                </div>
                            )}
                                            ...
                                            {cronBuilder.weeklyType === 'specific' && (
                                                <div>
                                                    ...
                                                    {cronBuilder.specificWeekdays.length === 0 && (
                                                        <div className="mt-2 rounded-lg border border-zinc-700/80 bg-zinc-950 p-2 text-xs text-zinc-400">
                                    ...
                                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 sm:px-3 sm:py-2"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-zinc-700 mb-2">Time</label>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="rounded-lg bg-zinc-100/80 p-2 sm:p-3">
                                <div className="mb-1 font-mono text-xs text-zinc-100 sm:text-sm">{generateCronExpression()}</div>
                                <div className="text-[11px] text-zinc-400 sm:text-xs">{getCronDescription()}</div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row">
                            <button
                                ...
                                className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-9500"
                            >
...
const ComponentNotesEdit = ({
    ...
    const [historyData, setHistoryData] = useState<{ executedTimeArr: string[]; totalExecuted: number; executedTimes: number; pendingLogs: Array<{ _id: string; taskType: string; taskStatus: string; createdAtUtc: string }> } | null>(null);
    const [historyLoading, setHistoryLoading] = useState(false);
    const initialSnapshotRef = useRef<string>(JSON.stringify({ title: taskScheduleObj.title, isActive: taskScheduleObj.isActive, taskType: taskScheduleObj.taskType, scheduleTimeArr: taskScheduleObj.scheduleTimeArr, cronExpressionArr: taskScheduleObj.cronExpressionArr }));
    useEffect(() => {
        const snap = JSON.stringify({ title: formData.title, isActive: formData.isActive, taskType: formData.taskType, scheduleTimeArr, cronExpressionArr, dueDateInput });
        if (snap !== initialSnapshotRef.current) {
            setIsDirty(true);
        } else {
            setIsDirty(false);
        }
    }, [formData.title, formData.isActive, formData.taskType, scheduleTimeArr, cronExpressionArr, dueDateInput]);
    useEffect(() => {
    ...
    useEffect(() => {
        const t = window.setTimeout(() => {
            saveDraft(taskScheduleObj._id, { title: formData.title, isActive: formData.isActive, taskType: formData.taskType, scheduleTimeArr, cronExpressionArr, dueDateInput });
        }, 1200);
        return () => window.clearTimeout(t);
    }, [formData.title, formData.isActive, formData.taskType, scheduleTimeArr, cronExpressionArr, dueDateInput, taskScheduleObj._id]);
    useEffect(() => {
        const d = loadDraft(taskScheduleObj._id);
        if (d && typeof d.title === 'string' && d.title !== '' && d.title !== taskScheduleObj.title) {
            toast(`Draft restored for "${String(d.title).slice(0, 30)}"`, { icon: '📝' });
        }
    }, [taskScheduleObj._id]);
    useEffect(() => {
        const onKey = (e: KeyboardEvent) => {
            if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
                if ((document.activeElement as HTMLElement)?.tagName === 'INPUT' || (document.activeElement as HTMLElement)?.tagName === 'TEXTAREA') {
                    e.preventDefault();
                    void editRecord();
                } else if (!isDirty) {
                    return;
    ...
    }, [isDirty, formData, scheduleTimeArr, cronExpressionArr]);
    const loadHistory = async () => {
        try {
            setHistoryLoading(true);
            const res = await taskScheduleHistoryAxios(taskScheduleObj._id, 1, 20);
            setHistoryData(res);
            setShowHistory(true);
        } catch (error) {
            console.error(error);
    ...
    const handleRunNow = async () => {
        try {
            await taskScheduleRunNowAxios(taskScheduleObj._id);
    ...
    const handleTestSend = async () => {
        try {
            const res = await taskScheduleTestSendAxios(taskScheduleObj._id);
    ...
    const handleDuplicate = async () => {
        try {
            const res = await taskScheduleDuplicateAxios(taskScheduleObj._id);
            if (res && res._id) {
    ...
    const copyEmailContent = async () => {
        try {
            await navigator.clipboard.writeText(formDataSendMyselfEmail.emailContent || '');
            toast.success('Copied email content');
        } catch {
    ...
    const editRecord = async () => {
        if (scheduleTimeArr.length === 0 && cronExpressionArr.length === 0 && (dueDateReminderPresetLabels.length === 0 || !dueDateInput)) {
            toast.error('Add at least one cron, time, or due-date reminder before saving');
        }
        setRequestEdit({
            loading: true,
            success: '',
        ...
        try {
            ...
            setIsDirty(false);
            try { localStorage.removeItem(getDraftKey(taskScheduleObj._id)); } catch { return; }
            initialSnapshotRef.current = JSON.stringify({ title: formData.title, isActive: formData.isActive, taskType: formData.taskType, scheduleTimeArr, cronExpressionArr, dueDateInput });
        } catch (error) {
            console.error(error);
    ...
    const renderEditFields = () => {
        return (
            <div className="space-y-3 sm:space-y-4">
                {/* field -> is active */}
                <div>
                    <label className="mb-2 block text-sm font-medium text-zinc-200 sm:mb-3">Is Active</label>
                    <div className="flex items-center">
                        <input
                            type="checkbox"
                            id="isActive"
                        ...
                        />
                        <label htmlFor="isActive" className="ml-2 text-sm text-zinc-400">
                            Is active
                        </label>
                    </div>
                </div>
                ...
                {/* field -> task type */}
                <div>
                    <label className="block text-sm font-medium text-zinc-200">Task Type *</label>
                    <select
                ...
                <div>
                    <label className="mb-2 block text-sm font-medium text-zinc-200 sm:mb-3">Send Email Notifications</label>
                    <div className="flex items-center">
                        <input
                            type="checkbox"
                            id="shouldSendEmail"
                        ...
                        />
                        <label htmlFor="shouldSendEmail" className="ml-2 text-sm text-zinc-400">
                ...
                {/* heading -> schedule */}
                <h2 className="my-3 border-b border-zinc-800 pb-2 text-lg font-semibold tracking-tight text-zinc-100 sm:my-4 sm:text-xl">
                    Schedule
                </h2>

                {/* field -> scheduleTimeArrTimezone */}
                <div>
                    <label className="block text-sm font-medium text-zinc-200">Schedule Timezone</label>
                    <select
                ...
                <div className="rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-2.5 sm:p-3">
                    ...
                    />
                    <div className="mt-3">
                        <div className="mb-2 text-sm font-medium text-zinc-200">Before due date reminders</div>
                ...
                />
                {cronExpressionArr.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                        {cronExpressionArr.map((c, i) => (
                            <span key={i} className="rounded-sm border border-zinc-700 bg-zinc-900 px-1.5 py-0.5 text-[11px] text-zinc-300">{cronExplain(c)}</span>
                        ))}
                    </div>
                )}
                {cronExpressionArr.length > 0 && taskScheduleObj.scheduleExecutionTimeArr.length > 0 && (
                    <div className="rounded-sm border border-indigo-800 bg-indigo-950/60 px-2 py-1 text-[11px] text-indigo-200">
                        <span className="font-medium text-indigo-300">Live next runs:</span> {taskScheduleObj.scheduleExecutionTimeArr.slice(0, 5).map((t) => new Date(t).toLocaleString()).join(' • ')}
                    </div>
                )}

                {/* field -> schedule execution times */}
                <div className="py-1.5 sm:py-2">
                    <label className="mb-1.5 block text-sm font-medium text-zinc-200 sm:mb-2">Schedule Execution Times</label>
                    <div className="max-h-[200px] overflow-y-auto rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-2 sm:p-3 [scrollbar-width:thin]">
                        {taskScheduleObj.scheduleExecutionTimeArr.length === 0 ? (
                            ...
                            <div className="space-y-1.5 sm:space-y-2">
                                {taskScheduleObj.scheduleExecutionTimeArr.map((time, index) => (
                                    <div key={index} className="rounded-lg border border-zinc-700/80 bg-zinc-900 p-2 text-xs text-zinc-700 sm:text-sm">
                ...
                <div className="py-1.5 sm:py-2">
                    <label className="mb-1.5 block text-sm font-medium text-zinc-200 sm:mb-2">Schedule Executed Times</label>
                    <div className="max-h-[200px] overflow-y-auto rounded-xl border border-zinc-700/80 bg-zinc-950/90 p-2 sm:p-3 [scrollbar-width:thin]">
                ...
                <div className="py-1.5 sm:py-2">
                    <label className="block text-sm font-medium text-zinc-200">Total Executed Times</label>
                ...
                <div className="flex flex-wrap gap-1.5 border-t border-zinc-800 pt-3">
                    <button type="button" aria-label="Run now" onClick={() => void handleRunNow()} className="inline-flex items-center gap-1 rounded-sm border border-emerald-700 bg-emerald-950 px-2 py-1 text-xs text-emerald-200 hover:bg-emerald-900"><LucidePlay className="h-3.5 w-3.5" />Run now</button>
                    <button type="button" aria-label="Duplicate" onClick={() => void handleDuplicate()} className="inline-flex items-center gap-1 rounded-sm border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-zinc-200 hover:bg-zinc-700"><LucideCopy className="h-3.5 w-3.5" />Duplicate</button>
                    <button type="button" aria-label="Test send" onClick={() => void handleTestSend()} className="inline-flex items-center gap-1 rounded-sm border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs text-zinc-200 hover:bg-zinc-800">Test email/Telegram</button>
                    <button type="button" aria-label="Preview email" onClick={() => setShowPreview(true)} className="inline-flex items-center gap-1 rounded-sm border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs text-zinc-200 hover:bg-zinc-800"><LucideEye className="h-3.5 w-3.5" />Preview</button>
                    <button type="button" aria-label="History" onClick={() => void loadHistory()} className="inline-flex items-center gap-1 rounded-sm border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-zinc-200 hover:bg-zinc-700"><LucideHistory className="h-3.5 w-3.5" />History</button>
                </div>
                {isDirty && (<div className="rounded-sm border border-amber-700 bg-amber-950 px-2 py-1 text-xs text-amber-200">Unsaved changes — draft auto-saved locally</div>)}
                {showPreview && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4" onClick={() => setShowPreview(false)}>
                        <div className="w-full max-w-lg rounded-xl border border-zinc-700 bg-zinc-900 p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
                            <div className="mb-2 flex items-center justify-between"><h3 className="text-sm font-semibold text-zinc-100">Email preview</h3><button type="button" aria-label="Close preview" className="rounded-sm border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-zinc-200" onClick={() => setShowPreview(false)}>Close</button></div>
                            <div className="rounded-sm border border-zinc-700 bg-zinc-950 p-2 text-sm text-zinc-200"><div className="mb-1 text-xs font-medium text-zinc-400">Subject: {formDataSendMyselfEmail.emailSubject || '(no subject)'}</div><div className="whitespace-pre-wrap text-xs text-zinc-300">{formDataSendMyselfEmail.emailContent || '(no content)'}</div></div>
                            <div className="mt-2 flex justify-end gap-1"><button type="button" aria-label="Copy email" className="rounded-sm border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-zinc-200" onClick={() => void copyEmailContent()}>Copy content</button><button type="button" aria-label="Test send from preview" className="rounded-sm bg-indigo-600 px-2 py-1 text-xs text-white" onClick={() => void handleTestSend()}>Test send</button></div>
                        </div>
                    </div>
                )}
                {(showHistory || historyLoading) && (
                    <div className="rounded-xl border border-zinc-700 bg-zinc-950 p-3">
                        <div className="mb-2 flex items-center justify-between"><h4 className="text-sm font-semibold text-zinc-100">Execution history</h4><button type="button" aria-label="Close history table" className="rounded-sm border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-zinc-200" onClick={() => setShowHistory(false)}>Close</button></div>
                        {historyLoading && (<div className="space-y-1">{[0, 1, 2].map((k) => (<div key={k} className="h-6 animate-pulse rounded-sm bg-zinc-800" />))}</div>)}
                        {!historyLoading && historyData && (
                            <Fragment>
                                <div className="mb-2 text-xs text-zinc-400">Total executed: <span className="font-semibold text-zinc-100">{historyData.executedTimes}</span> • {historyData.totalExecuted} rows</div>
                                <div className="max-h-40 overflow-y-auto rounded-sm border border-zinc-700">
                                    <table className="w-full text-left text-xs">
                                        <thead className="bg-zinc-800 text-[11px] uppercase text-zinc-400"><tr><th className="px-2 py-1">#</th><th className="px-2 py-1">Time</th></tr></thead>
                                        <tbody>
                                            {historyData.executedTimeArr.length === 0 && (<tr><td colSpan={2} className="px-2 py-2 text-center text-zinc-500">No executions yet</td></tr>)}
                                            {historyData.executedTimeArr.map((iso, idx) => (<tr key={idx} className="border-t border-zinc-800 text-zinc-200"><td className="px-2 py-1">{idx + 1}</td><td className="px-2 py-1">{new Date(iso).toLocaleString()}</td></tr>))}
                                        </tbody>
                                    </table>
                                </div>
                                <div className="mt-2 rounded-sm border border-zinc-700 bg-zinc-900 p-2">
                                    <div className="text-xs font-medium text-zinc-200">Pending task log</div>
                                    {historyData.pendingLogs.length === 0 && (<div className="text-xs text-zinc-500">No logs</div>)}
                                    {historyData.pendingLogs.map((log) => (<div key={log._id} className="flex justify-between border-t border-zinc-800 py-1 text-[11px]"><span className="text-zinc-300">{log.taskType}</span><span className={`rounded-sm px-1 py-0.5 text-[10px] border ${getLogStatusClassEdit(log.taskStatus)}`}>{log.taskStatus}</span></div>))}
                                </div>
                            </Fragment>
                        )}
                    </div>
    ...
    return (
        <div className="rounded-xl border border-zinc-700/80 bg-zinc-900 p-2 shadow-sm sm:p-3 md:p-4">
            <Helmet><title>Edit Schedule - {taskScheduleObj.title}</title></Helmet>
            {requestEdit.loading && (
                <div className="mb-3 flex justify-between border-b border-zinc-800 pb-2">
                    <span className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-1 text-xs font-medium text-zinc-400">
            ...
            {!requestEdit.loading && (
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-zinc-800 pb-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        ...
                        <button
                            type="button"
                            className="inline-flex items-center gap-1 rounded-lg border border-indigo-600/20 bg-indigo-600 px-2 py-1 text-xs font-medium text-white hover:bg-indigo-9500"
                            onClick={() => {
                                editRecord();
                            }}
                            aria-label="Save"
...
const ComponentNotesEditWrapper = ({
    ...
    return (
        <div>
            {loading && (
                <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-zinc-700 bg-zinc-900 py-10 shadow-sm">
                    <div className="h-6 w-6 animate-spin rounded-full border-2 border-zinc-700 border-t-indigo-600" />
                    <p className="text-xs text-zinc-400">Loading…</p>
                </div>
            )}
            {!loading && list.length === 0 && (
                <div className="rounded-lg border border-zinc-700 bg-zinc-900 px-4 py-8 text-center shadow-sm">
                    <p className="text-sm font-medium text-red-700">Record does not exist.</p>
                    <button
                        type="button"
                        className="mt-3 inline-flex rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-xs font-medium text-zinc-200 hover:bg-zinc-800"
