...
interface WorkspaceModalProps {
    isOpen: boolean;
    onClose: () => void;
    editingWorkspace: ITaskWorkspace | null;
    onSuccess: () => void;
}
...
const WorkspaceModal = ({ isOpen, onClose, editingWorkspace, onSuccess }: WorkspaceModalProps) => {
        ...
        try {
            const config = {
                ...
                url: editingWorkspace 
                    ? '/api/task-workspace/crud/taskWorkspaceEdit'
                    : '/api/task-workspace/crud/taskWorkspaceAdd',
                headers: {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-zinc-900 rounded-sm p-6 w-full max-w-md mx-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2 rounded-sm border border-zinc-700 bg-zinc-800 text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        <button
                            ...
                            onClick={handleClose}
                            disabled={loading}
                            className="px-4 py-2 text-zinc-300 border border-zinc-700 rounded-sm hover:bg-zinc-800 transition-colors disabled:opacity-50"
                        >
                            Cancel
                        </button>
                        <button
