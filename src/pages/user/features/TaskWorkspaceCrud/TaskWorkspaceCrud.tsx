...
const TaskWorkspaceCrud = () => {
    const [taskWorkspace, setTaskWorkspace] = useState<ITaskWorkspace[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [modalOpen, setModalOpen] = useState<boolean>(false);
    const [editingWorkspace, setEditingWorkspace] = useState<ITaskWorkspace | null>(null);
    ...
    const fetchWorkspaces = async () => {
        try {
            const result = await axiosCustom.post<{ docs: ITaskWorkspace[] }>('/api/task-workspace/crud/taskWorkspaceGet');
            setTaskWorkspace(result.data.docs);
        } catch (err) {
    ...
    const openModal = (workspace?: ITaskWorkspace) => {
    ...
    const handleDelete = async (workspace: ITaskWorkspace) => {
        if (!window.confirm(`Delete "${workspace.title}"?`)) return;

        try {
            await axiosCustom.post('/api/task-workspace/crud/taskWorkspaceDelete', { _id: workspace._id });
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="animate-pulse">
                        ...
                        <div className="grid gap-3 sm:gap-4">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="h-24 sm:h-32 bg-zinc-900 rounded-xl shadow-sm"></div>
                            ))}
                        </div>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-8">
                    <div className="text-center sm:text-left">
                        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-zinc-100 mb-1 sm:mb-2">Workspaces</h1>
                        <p className="text-sm sm:text-base text-zinc-400">Organize your notes in beautiful workspaces</p>
                    </div>
                    <button
                        onClick={() => openModal()}
                ...
                {taskWorkspace.length === 0 ? (
                    <div className="text-center py-12 sm:py-16">
                        <div className="bg-zinc-900 rounded-2xl shadow-lg p-8 sm:p-12 max-w-sm sm:max-w-md mx-auto">
                            ...
                            <h3 className="text-lg sm:text-xl font-semibold text-zinc-100 mb-2">No workspaces yet</h3>
                            <p className="text-sm sm:text-base text-zinc-400 mb-6">Create your first workspace to get started!</p>
                            <button
                                onClick={() => openModal()}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                        {taskWorkspace.map((workspace) => (
                            <div key={workspace._id} className="bg-zinc-900 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
                                <div className="p-4 sm:p-6">
                                    <h3 className="text-lg sm:text-xl font-bold text-zinc-100 mb-2">{workspace.title}</h3>
                                    <p className="text-sm sm:text-base text-zinc-400 mb-4 sm:mb-6 line-clamp-2 min-h-[2.5rem] sm:min-h-[3rem]">{workspace.description}</p>
...
export default TaskWorkspaceCrud;