...
const MapSearchProduct = () => {
    ...
    const [showIndiaOutline] = useState(true);
    ...
    const renderSearchBar = () => (
        <div className="flex flex-wrap gap-2 items-center">
            <DebounceInput
                ...
                className="flex-1 min-w-[160px] bg-zinc-900 text-zinc-200 shadow-sm rounded-sm border border-zinc-700 h-10 px-3 focus:border-blue-400 focus:outline-none transition-colors"
            />

            <span
                className="inline-flex items-center px-2 h-10 bg-zinc-900 rounded-sm border border-zinc-700 text-xs text-zinc-400 tabular-nums"
                title="Result count"
            >
                {resultLabel}
            </span>
            ...
            <button
                type="button"
                className="px-3 py-1 border border-blue-400 text-blue-400 rounded-sm text-sm hover:bg-zinc-800 transition-colors"
            ...
            <button
                type="button"
                className="px-3 py-1 border border-zinc-600 text-zinc-400 rounded-sm text-sm hover:bg-zinc-800 transition-colors"
                onClick={() => {
                    hasFittedBoundsRef.current = false;
                    updateMapMarkers();
                }}
    ...
    const renderList = () => (
        ...
        >
            ...
            {error && (
                <div className="mb-3 text-sm text-red-400 bg-red-950/30 border border-red-900 rounded-sm px-3 py-2">
                    {error}
                </div>
            )}

            <div className="py-2">
                <h3 className="text-sm font-semibold text-zinc-300">{resultLabel}</h3>
            </div>

            <div className="pb-5">
                {!loading && mapsData.length === 0 && (
                    <div className="text-center text-zinc-500 text-sm py-10 px-2">
                    ...
                    return (
                        <button
                            ...
                            className={`w-full text-left bg-gradient-to-r rounded-sm p-4 mb-3 border shadow-sm transition-all duration-200 cursor-pointer ${
                                active
                                    ? 'from-zinc-700 to-zinc-800 border-amber-600 ring-1 ring-amber-700'
                                    : 'from-zinc-800 to-zinc-900 border-zinc-700 hover:shadow-md'
                            }`}
                        >
                            <div className="flex items-center space-x-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-lg font-semibold text-zinc-200 mb-1 truncate">
                                        {mapDoc.lifeEvents.name || 'Untitled'}
                                    </h3>
                                    <div className="flex flex-wrap items-center gap-2">
                                        {mapDoc.lifeEvents.infoVaultType && (
                                            <span className="px-2 py-0.5 bg-zinc-700 text-blue-400 text-xs rounded-sm font-medium">
                                                {mapDoc.lifeEvents.infoVaultType}
                                            </span>
                                        )}
                                        {addressLine && (
                                            <span className="text-zinc-400 text-sm truncate">{addressLine}</span>
                                        )}
                                    </div>
                                </div>
                            </div>
    ...
    return (
        <div className="w-full bg-zinc-950 p-0">
            {currentScreen === 'sm' && <div className="p-2 border-b border-zinc-800 bg-zinc-900">{renderSearchBar()}</div>}
            ...
            >
                {loading && (
                    <div className="absolute inset-0 z-[500] bg-zinc-950/50 flex items-center justify-center pointer-events-none">
                        <span className="text-sm text-zinc-400 bg-zinc-900 px-4 py-2 rounded-sm shadow border border-zinc-700">
                            Loading map…
                        </span>
                    </div>
                )}
