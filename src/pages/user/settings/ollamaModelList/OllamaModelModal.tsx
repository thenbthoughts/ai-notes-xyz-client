...
interface OllamaModelModalProps {
    isOpen: boolean;
    onClose: () => void;
    editingModel: OllamaModel | null;
    onSuccess: () => void;
}

const OllamaModelModal: React.FC<OllamaModelModalProps> = ({
    isOpen,
    onClose,
    editingModel,
    onSuccess,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const config: AxiosRequestConfig = {
                method: 'patch',
                url: '/api/dynamic-data/model-ollama/modelOllamaUpdate',
            ...
            toast.success('Ollama model updated successfully!');
            onSuccess();
            onClose();
        } catch (error: any) {
            console.error(error);
            toast.error(error?.response?.data?.message || 'Failed to update Ollama model');
        } finally {
            setLoading(false);
        }
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="e.g. llama3.1:8b (8.0GB)"
                            disabled={loading}
                            required
                        />
                    </div>
