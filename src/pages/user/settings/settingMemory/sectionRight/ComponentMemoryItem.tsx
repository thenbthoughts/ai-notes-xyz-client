...
const ComponentMemoryItem = ({ memoryObj, onUpdate, onDelete }: ComponentMemoryItemProps) => {
    ...
    return (
        <div className="mb-3 p-3 sm:p-4 bg-zinc-900 border border-zinc-700 rounded-sm shadow-sm hover:shadow-md transition-shadow">
            {isEditing ? (
                <div>
                    <textarea
                        value={content}
                        onChange={(e) => setContent(e.target.value)}
                        className="w-full p-2 text-sm sm:text-base border border-zinc-700 bg-zinc-800 text-zinc-200 rounded-sm mb-3 min-h-[80px] resize-y"
                    ...
                    <div className="flex items-center gap-4 mb-3">
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                className="w-4 h-4"
                            />
                            <span className="text-xs sm:text-sm text-zinc-300">Protect from deletion</span>
                        </label>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className="flex items-center justify-center gap-2 px-3 py-1.5 bg-zinc-600 text-white rounded-sm hover:bg-zinc-700 disabled:opacity-50 text-sm sm:text-base"
                        >
                ...
                <div>
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-2 gap-2">
                        <div className="flex-1 min-w-0">
                            <p className="text-sm sm:text-base text-zinc-200 leading-relaxed break-words">{memoryObj.content}</p>
                        </div>
                        <div className="flex items-center gap-2 sm:ml-4 flex-shrink-0">
                            ...
                            {!memoryObj.isPermanent && (
                                <div className="flex items-center gap-1 px-2 py-1 bg-zinc-800 text-zinc-400 rounded-sm text-xs whitespace-nowrap">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-3 pt-3 border-t border-zinc-700 gap-2">
