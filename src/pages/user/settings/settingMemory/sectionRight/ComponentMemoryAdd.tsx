...
const ComponentMemoryAdd = ({ onAdd }: ComponentMemoryAddProps) => {
    ...
    return (
        <div className="mb-4 p-3 sm:p-4 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-sm shadow-sm">
            <h3 className="text-base sm:text-lg font-semibold text-zinc-200 mb-3">Add New Memory</h3>
            <textarea
                value={content}
                onChange={(e) => setContent(e.target.value)}
                className="w-full p-2 text-sm sm:text-base border border-zinc-700 rounded-sm mb-3 min-h-[100px] resize-y"
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                ...
                <button
                    ...
                    disabled={isLoading}
                    className="px-4 py-2 bg-zinc-600 text-white rounded-sm hover:bg-zinc-700 disabled:opacity-50 text-sm sm:text-base"
                >
                    Cancel
                </button>
            </div>
