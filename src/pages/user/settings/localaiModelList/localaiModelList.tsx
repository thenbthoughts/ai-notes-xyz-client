...
import LocalaiModelModal from './LocalaiModelModal';
...
const LocalaiModelList: React.FC = () => {
    ...
    // Edit modal state
    const [editingModel, setEditingModel] = useState<LocalaiModel | null>(null);
    ...
    const updateContextLength = async (_id: string, contextLength: number) => {
        try {
            setLoading(true);
            await axiosCustom.patch('/api/dynamic-data/model-localai/modelLocalaiUpdate', {
                _id,
            ...
            setError(err.response?.data?.message || 'Failed to update context length');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const updateMaxCompletionTokens = async (_id: string, maxCompletionTokens: number) => {
        try {
            setLoading(true);
            await axiosCustom.patch('/api/dynamic-data/model-localai/modelLocalaiUpdate', {
                _id,
            ...
            setError(err.response?.data?.message || 'Failed to update max completion tokens');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const openEditModal = (model: LocalaiModel) => {
    ...
    const filteredModels = useMemo(() => {
        return models.filter((model) => {
            ...
            const matchesType =
                ...
                (selectedTypeFilter === 'unspecified' && !model.modelType) ||
                model.modelType === selectedTypeFilter;

            const matchesModality = matchModalityFilters(model, modalityFilters);

            return matchesSearch && matchesType && matchesModality;
        });
    }, [models, searchQuery, selectedTypeFilter, modalityFilters]);
        ...
        <div className="localai-model-list w-full max-w-4xl">
            ...
            <div className="flex items-center justify-between mb-2 sm:mb-3">
                <h2 className="text-2xl font-extrabold tracking-tight text-zinc-100">
                    <span className="text-blue-500">LocalAI</span> Model Manager
                </h2>
            </div>
            ...
            <div className="mb-3 sm:mb-4 rounded-lg border border-blue-800/60 bg-blue-950/40 p-2 sm:p-3">
                <div className="mb-2">
                    <h3 className="mb-1 text-sm font-semibold text-blue-200">💡 How to add models:</h3>
                    <div className="space-y-0.5 text-xs text-blue-300/90">
                        <p>Configure models in your LocalAI instance. Visit <a href="https://localai.io/" target="_blank" rel="noopener noreferrer" className="underline text-blue-400 hover:text-blue-300">localai.io</a> for setup instructions.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="mb-3 rounded-lg border border-zinc-700 bg-zinc-900/80 p-3 space-y-3">
                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center justify-between">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            placeholder="Search models (regex word break e.g. 'whisper stt')..."
                    ...
                    <div className="flex items-center gap-2">
                        <select
                            ...
                            className="px-2.5 py-1.5 rounded-md text-xs font-medium border border-zinc-700 bg-zinc-800 text-zinc-200 focus:ring-1 focus:ring-blue-500"
                        >
                            ...
                            <option value="image-generation">Image Gen</option>
                ...
                {showFilters && (
                    <div className="pt-2 border-t border-zinc-800 space-y-2 text-xs">
                        ...
                        {(hasActiveModalityFilters(modalityFilters) || searchQuery || selectedTypeFilter !== 'all') && (
                            <div className="pt-1">
                                <button
                                    type="button"
                ...
                {loading && models.length === 0 ? (
                    ...
                    <div className="text-center py-6 bg-zinc-900 rounded-lg border border-zinc-800 text-zinc-400">
                        <p className="text-sm sm:text-base">
                            {models.length === 0
                                ? 'No models found. Configure models in your LocalAI instance and click "Refresh Models" above.'
                    ...
                    <div>
                        ...
                        <ul className="space-y-1 sm:space-y-1.5">
                            {filteredModels.map((model) => {
                                ...
                                const modelTypeTagClass = model.modelType ? 'bg-indigo-950 text-indigo-300 border border-indigo-800' : 'bg-zinc-800 text-zinc-400 border border-zinc-700';
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-2 sm:p-3 gap-1 sm:gap-0">
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                                                    ...
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${modelTypeTagClass}`}>
                                                        {modelTypeLabel}
                                                    </span>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                                    {rawData.created && (
                                                        <div>
                                                            ...
                                                            <div className="text-zinc-400">{formatDate(rawData.created)}</div>
                                                        </div>
                                                    )}
                                                    {rawData.owned_by && (
                                                        <div>
                                                            <span className="font-medium text-zinc-300">Owned by:</span>
                                                            <div className="text-zinc-400">{rawData.owned_by}</div>
                                                        </div>
                                                    )}
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="mt-1 flex items-center gap-2">
                                                            <input
                                                                ...
                                                                onChange={(e) => updateContextLength(model._id, parseInt(e.target.value) || 0)}
                                                                disabled={loading}
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="mt-1 flex items-center gap-2">
                                                            <input
                                                                ...
                                                                onChange={(e) => updateMaxCompletionTokens(model._id, parseInt(e.target.value) || 0)}
                                                                disabled={loading}
                                                                className="w-28 text-xs border border-zinc-700 rounded px-2 py-1 bg-zinc-800 text-zinc-200 focus:ring-1 focus:ring-blue-500"
                                                            />
                                                            {maxTokens > 0 && (
                                                    ...
                                                    {/* Model Type */}
                                                    <div>
                                                        <span className="font-medium text-zinc-300">Model Type:</span>
                                                        <div className="mt-1">
                                                            <select
                                                                value={model.modelType || ''}
                                                                onChange={(e) => updateModelType(model._id, e.target.value as LocalaiModelType)}
                                                                disabled={loading}
                                                                className="text-xs border border-zinc-700 rounded-md px-2 py-1 bg-zinc-800 text-zinc-200 focus:ring-1 focus:ring-blue-500"
                                                            >
                                                        ...
                                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-1">
                                                            {[
                                                                ...
                                                                <div key={key} className="flex items-center gap-1.5">
                                                                    ...
                                                                    <select
                                                                        value={normalizeModalityValue((model as any)[key])}
                                                                        onChange={(e) => updateModality(model._id, key as any, e.target.value as any)}
                                                                        disabled={loading}
                                                    ...
                                                    {rawData.id && (
                                                        <div className="col-span-1 sm:col-span-2">
                                                            ...
                                                            <div className="text-zinc-400 font-mono text-xs break-all">{rawData.id}</div>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
