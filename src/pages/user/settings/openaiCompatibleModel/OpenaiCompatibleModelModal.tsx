...
const OpenaiCompatibleModelModal = ({ isOpen, onClose, editingConfig, onSuccess }: OpenaiCompatibleModelModalProps) => {
    ...
    useEffect(() => {
        if (editingConfig) {
            setFormData({
                ...
                contextLength: String(editingConfig.contextLength || 0),
                maxCompletionTokens: String(editingConfig.maxCompletionTokens || 0),
                ...
                isOutputModalityEmbedding: editingConfig.isOutputModalityEmbedding || 'false',
            });
        } else {
            setFormData({
                providerName: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const hasOutputModality = formData.isOutputModalityText === 'true' || 
                                  ...
                                  formData.isOutputModalityEmbedding === 'true';
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-zinc-900 rounded-sm p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button
                        onClick={handleClose}
                        disabled={loading}
                        className="p-1 text-zinc-400 hover:text-zinc-400 transition-colors disabled:opacity-50"
                    >
                        <LucideX size={20} />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-3 py-2 border border-zinc-700 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            placeholder="e.g., OpenRouter, LM Studio, Local LLM"
                            disabled={loading}
                        />
                        <p className="text-zinc-400 text-xs mt-1">A friendly name to identify this provider</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex items-center mt-2">
                            <input
                                ...
                                disabled={loading}
                            />
                            <label htmlFor="showApiKey" className="ml-2 text-sm text-zinc-300 cursor-pointer">
                                Show API Key
                            </label>
                        </div>
                        <p className="text-zinc-400 text-xs mt-1">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={loading}
                        />
                        <p className="text-zinc-400 text-xs mt-1">The specific model to use from this provider</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={loading}
                        />
                        <p className="text-zinc-400 text-xs mt-1">Max context window for this model in tokens (0 = not specified)</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={loading}
                        />
                        <p className="text-zinc-400 text-xs mt-1">Max output tokens for this model (0 = not specified)</p>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-3 py-2 border border-zinc-700 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-sm"
                    ...
                    <div>
                        ...
                        <p className="text-zinc-400 text-xs mt-1">Select the input types this model supports</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            ...
                            <label className="flex items-center space-x-2 cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={formData.isOutputModalityEmbedding === 'true'}
                                    onChange={(e) => setFormData({ ...formData, isOutputModalityEmbedding: e.target.checked ? 'true' : 'false' })}
                                    className="w-4 h-4 text-blue-600 border-zinc-700 rounded focus:ring-blue-500"
                                    disabled={loading}
                                />
                                <span className="text-sm text-zinc-300">Embedding</span>
                            </label>
                        </div>
                        <p className="text-zinc-400 text-xs mt-1">Select the output types this model supports</p>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        <button
                            ...
                            onClick={handleClose}
                            disabled={loading}
                            className="px-4 py-2 text-zinc-400 border border-zinc-700 rounded-sm hover:bg-zinc-800 transition-colors disabled:opacity-50"
                        >
                            Cancel
                        </button>
                        <button
