...
import { panel, panelTitle, mutedText } from './homepagePanelStyles';
...
const ComponentFromBrithdayToToday = () => {
    ...
    useEffect(() => {
        const calculateAgeAndTimeRemaining = () => {
            ...
            setNextBirthdayWeekday(nextBirthdayDate.toFormat('cccc'));
    ...
    return (
        <div className="space-y-2">
            {birthday !== '0000-00-00' && (
                <Fragment>
                    <div className={`${panel} border-l-4 border-l-sky-400`}>
                        ...
                        <p className="text-base font-extrabold leading-snug text-sky-100">
                            <span className="tabular-nums">{age.years}</span>{' '}
                            <span className="text-sm font-semibold text-sky-400/80">
                    ...
                    <div className={`${panel} border-l-4 border-l-cyan-400`}>
                        <h2 className={`${panelTitle} mb-1`}>
                            <LucideCake className="h-3.5 w-3.5 text-cyan-400" strokeWidth={2} />
                            Next birthday
                        </h2>
                        <p className="text-xs font-semibold text-sky-200">
                            {nextBirthdayWeekday ? `${nextBirthdayWeekday}, ${nextBirthday}` : nextBirthday}
            ...
            {birthday === '0000-00-00' && (
                <Link
                    to="/user/setting"
                    className={`${panel} block border-l-4 border-l-rose-400 transition hover:bg-rose-950/40`}
                >
